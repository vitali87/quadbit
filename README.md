...
- **Dense FP4, zero training, any model — real cost is W4A4, not the weight-only figure.** The FP4 tensor core multiplies fp4×fp4, so the deployed kernel is **W4A4** (weights *and* activations 4-bit); there is no weight-only FP4 GEMM path in hardware. Measured through the deploy-matched quantizer on wikitext-2 (MLP linears): **Llama-3-8B +1.9 PPL (6.20→8.09), TinyLlama-1.1B +2.2 (7.53→9.73)**. The commonly quoted **+0.3 PPL is weight-only (W4A16)** — Sparse-Llama-3.1-8B 7.89→8.16, Qwen2.5-3B 7.60→7.91, and reproduced here as **+0.29 on Llama-3-8B, +0.20 on TinyLlama** — but W4A16 is not what ships. Dense FP4 is still a zero-training drop-in; its honest deployed cost is ~+2 PPL. (Activation quant here is single-level per-32 NVFP4; two-level activation scaling may recover some of the W4A4 gap — not yet measured.)
...
- **Sparse FP4** needs recovery, because Blackwell FP4 2:4 is **pair-granular** (the mma selects 2 of every 4 fp4 pairs), unlike existing element-granular 2:4 checkpoints. The pipeline (pair-granular SparseGPT one-shot prune, then knowledge distillation from the dense teacher, then QAT with straight-through fake-quant of both weights and activations) recovers TinyLlama-1.1B to **9.60 PPL through the real sparse FP4 kernel** (dense teacher 7.53). The straight-through fake-quant bit-matches the deployed activation quantizer, so QAT trains against exactly what ships: the fake-quant eval (9.57) tracks the through-kernel PPL (9.60) within 0.04. This is monotonic in training data, so production parity is a data-scale question, not a method gap.

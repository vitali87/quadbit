...
**C2 SOTA board: the dense NVFP4 fused MoE is the SM120 MoE decode SOTA, not quadbit sparse.** A direct same-harness board (same passage, decode-only formula, graph mode) put the sparse D2 policy against the strongest dense/NVFP4 baseline that runs on SM120 (vLLM's native FlashInfer-CUTLASS fused NVFP4 MoE, `QB_MOE=off`, attention/DSA still plugin-unblocked). The dense baseline wins decode and memory on both models:

| model | dense NVFP4 fused (captured) | quadbit D2 native (captured) | dense advantage |
|---|---|---|---|
| DeepSeek (4 GPU) | 48.248 tok/s, 40.83 GiB/GPU | 5.972 tok/s, 51.7 GiB/GPU | 8.1× decode, less memory |
| GLM (8 GPU) | 33.810 tok/s, 54.62 GiB/GPU | 5.367 tok/s, 68.98 GiB/GPU | 6.3× decode, less memory |

**These decode numbers are platform-bound, not kernel-bound.** Re-running the GLM-5.2 dense row on **4× B200 (NVLink)** with the same checkpoint, harness, passage and commit, and with every quadbit patch disabled (vanilla vLLM), gives **112.079 tok/s vs 34.305 on 8× RTX PRO 6000 (PCIe) = 3.27×**, with neither row collapsing on quality (PPL 3.7352 vs 3.9168, too few scored tokens to attribute the delta) — and with no kernel, quantization, or policy change in quadbit. That is direct confirmation of C4's profiler finding that GEMM+MoE are 2.2% of the SM120 decode step while one per-layer PCIe all-reduce is 90.8%. It is also only *partial*: the collective is still ~70% of the B200 step, so the floor moved rather than vanished. Adding **MTP speculative decoding** — which C9 measured as a **KILL** on SM120 (−17.8% at spec=2), because there each draft step paid a 374 µs PCIe all-reduce — now gives **+93.5% at the same spec=2 (216.828 tok/s), losslessly** (PPL identical to 4dp), for **6.32× cumulative** over the PCIe row. Read the SM120 serving KILLs (C7, C8, C9) as **conditional on that PCIe floor**, not as general properties of those techniques. See [docs/b200/verdict.md](docs/b200/verdict.md).

C1's 11.3× was measured against the crippled dequant loop (0.514 tok/s), not this dense path. So quadbit's sparse-MoE contribution is **not** decode speed: it is the only deployed 2:4-sparse FP4 MoE, training-free quality-preserving structural sparsity that transfers across architectures and graph-captures with native DSA (downstream within ~1 pt), and a prefill/large-M *kernel* Pareto point (§5, not a serving-decode result). The remaining MoE decode gap is a kernel problem (a fused sparse grouped decode GEMM at tiny M), identified but not built. See [docs/c2/verdict.md](docs/c2/verdict.md).

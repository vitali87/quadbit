...
- **Dense FP4 cost is recipe-dependent.** The FP4 tensor core multiplies fp4×fp4, so the deployed kernel is **W4A4** (weights *and* activations 4-bit); the often-quoted +0.3 PPL is weight-only (W4A16), which the hardware never runs. Our current uncalibrated per-32 single-level activation quant costs about **+2 PPL**; the calibrated per-16 two-level NVFP4 recipe (modelopt reference) reaches **+0.7**, and our kernel can consume those scales directly. **+0.7 is the real W4A4 floor**; matching it in our own recipe is in progress. Validated against the reference: vLLM native NVFP4 scores **7.97 vs bf16 teacher 7.27** on Llama-3.1-8B (same wikitext-2 windows), i.e. +0.70.

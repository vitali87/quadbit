...
**Honest limits for the MoE path:** the quadbit vLLM plugin path is **graph-enabled, speed-limited by the dense grouped path** (P4) — the old plugin host-sync (`torch.unique(...).tolist()`) was replaced by a fixed-capacity device-routing path, so the deployed sparse policies now **CUDA-graph-capture correctly in vLLM on SM120** (DeepSeek-D2 on 2 GPUs, GLM route-slot D2 on 8 GPUs, native SM120 DSA sparse-MLA, quality-neutral, `drop=0`). The remaining limitation is **not** graph capture; it is decode speed on the **unfused dense anchored-projection path**, which still lacks a native dense NVFP4 grouped-GEMM, so the DeepSeek/GLM serving numbers are a graph-correctness result, not a production-wide decode-speed win (not a DSA/attention/memory/loader blocker). **GLM-5.2 requires 8× RTX PRO 6000** (it does not fit on 2 or 4). The **GLM downstream evidence is a 4-task smoke suite on the D2 policy, not an exhaustive benchmark** — no claim of exhaustive GLM downstream preservation; the fullest downstream accounting of record remains DeepSeek's. MoE per-expert 2:4-FP4 accuracy recovery beyond structural placement is future work. See [docs/paper.md](docs/paper.md) §10, [docs/glm_results.md](docs/glm_results.md), `harness/moe_seg.py`, `harness/serve_dsv4.py`, `docs/figures/`.

...
**Distributed sparse-FP4 MoE on a large model (DeepSeek-V4-Flash, 284B/13B).** The approach transfers from one dense Llama-8B to a 256-expert MoE across GPUs. The experts are **MXFP4** (not the config's FP8); we decode them value-exact (100% round-trip) and prune 2:4 + re-quantize to two-level NVFP4. A **segmented routed-row kernel** (`matmul_sp_moe`, one launch, expert id per column-block) makes MoE serving **CUDA-graph capturable** — validated bit-exact vs the per-expert kernel (**cos 1.000000**, all routing patterns, and on one real 256-expert layer) with graph capture/replay at cos 1.0. quadbit sparsifies **~91% of parameters / ~80% of active linear FLOPs** (all expert MLPs; attention/router/embeddings stay dense). Expert-parallel scaling is near-linear: **2.17x (2 GPU) / 4.21x (4 GPU)** of expert-kernel time, routing imbalance 1.04, checksum-identical output; on no-NVLink PCIe the all-reduce is 0.32–0.45 ms vs 1.3–2.5 ms compute. The dense NVFP4 baseline serves in vLLM 0.24 on 2× RTX PRO 6000; the in-vLLM sparse injection uses the staged-`.so` pattern (sparse mma assembles only under CUDA ≤12.8). Per-expert-output 2:4-FP4 tax is ~cos 0.70 — again a **speed-for-capability** point; MoE accuracy recovery is future work. See [docs/paper.md](docs/paper.md) §10, `harness/moe_seg.py`, `harness/moe_layer.py`, `harness/moe_dist.py`, `docs/figures/`.

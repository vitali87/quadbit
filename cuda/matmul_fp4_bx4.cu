// Dense FP4 pingpong, LDSM-reduced mainloop. Same as matmul_fp4_pp_bf16 but the B
// operand is loaded with 4x ldmatrix.m8n8.x4 (2 n-tiles per instruction) instead of
// 8x ldmatrix.m8n8.x2. That cuts per-k-step LDSM from 12 (4 A.x4 + 8 B.x2) to 8
// (4 A.x4 + 4 B.x4): the ILP-reorder was neutral (ptxas schedules OMMA optimally), so
// the only remaining lever on the ~14% gap to the register-only mma peak (1811k) is
// issuing fewer LDSM instructions that steal issue slots from OMMA. Fragment mapping
// is byte-identical to the x2 version; only the load-instruction count changes.
...
#pragma unroll
            ...
            int grp = lane >> 3, row = lane & 7;  // grp 0..3 -> 4 matrices of the x4 load
#pragma unroll
            for (int np = 0; np < 4; np++) {
                // one x4 loads 2 adjacent n-tiles (cols np*16..np*16+15): matrices 0,1 =
                // n-tile 2*np (rows +0), matrices 2,3 = n-tile 2*np+1 (rows +8); even/odd
                // matrix = byte kb+0 / kb+16 == the x2 version's bf[n][0]/bf[n][1].
                int bcol = wn * 64 + np * 16 + (grp >> 1) * 8 + row;
                int bo = bcol * BKH + kb + (grp & 1) * 16;
                bo ^= ((bo >> 7) & 3) << 4;
...
void run(int sz) {
    ...
    int wrong = 0;
    for (size_t i = 0; i < (size_t)M * N; i++) if (__bfloat162float(hC[i]) != (float)Kf) wrong++;
    printf("matmul_fp4_bx4: %dx%dx%d  %.3f ms  %.1f GFLOP/s  %s (%d wrong)\n",

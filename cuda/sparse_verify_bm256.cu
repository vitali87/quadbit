...
// Real-data correctness for the 256x128 shared-B kernel: random pair-granular 2:4 A
// (compressed) and random B, unit ue4m3 scales. Validates the new tiling's row/col ->
// global output mapping, which all-ones cannot (a transposed map still sums to Klog/2).
void run(int sz) {
    ...
    for (int i = 0; i < M; i++)
        for (int cs = 0; cs < KAb; cs++) {            // compressed pair index (byte)
    ...
    printf("sparse_verify_bm256 (256x128 shared-B, random A/B, unit scales): %dx%dx%d  %s (%d wrong, maxrel %.4f)\n",

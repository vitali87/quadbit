// Raw-PTX track, OMMA frontier: match CUTLASS's tile shape. One CUTLASS pingpong
// "warpgroup" = 4 warps computing a 128x128 tile, each warp a 64x64 sub-tile = 4x8
// = 32 mma (128 f32 accumulators), high ILP. Builds on the 1262k swizzled TMA kernel
// (matmul_fp4_swz): same 64B swizzle + 3-stage TMA ring, but BM=128 and the wider
// warp tile. Needs dynamic smem (3*(8192+8192)=48KB > 48KB static cap). All-consumer
// (thread0 issues TMA); the 2nd warpgroup (pingpong) comes next if this pays.
...
#define SMEM (STAGES * ASZ + STAGES * BSZ + 64)  // + room for barriers (8-aligned)
...
matmul_fp4(const __grid_constant__ CUtensorMap mapA, const __grid_constant__ CUtensorMap mapB,
    ...
    uint64_t *full = (uint64_t *)(smem + STAGES * ASZ + STAGES * BSZ);
...
void run(int sz) {
    ...
    printf("matmul_fp4_big: %dx%dx%d  %.3f ms  %.1f GFLOP/s  %s (%d wrong)\n",

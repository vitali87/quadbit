...
#pragma unroll
            ...
            acc[0][0]+=(float)a_s[aoff+wtid]+(float)b_s[boff+wtid]+(float)scA[0]+(float)mtA[0];
        }
...
void run(int sz, bool ref) {
    ...
    if(ref){int wrong=0;float maxrel=0;
        ...
        printf("matmul_sp_fullw_mem VERIFY %dx%dx%d: %s (maxrel %.4f)\n",sz,sz,sz,wrong==0?"PASS":"FAIL",maxrel);
    } else printf("matmul_sp_fullw_mem %dx%dx%d: %.3f ms  %.1f GFLOP/s\n",sz,sz,sz,ms,gf);
...
int main(){ run(8192,false); return 0; }

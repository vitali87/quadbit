...
#pragma unroll
            for (int mt = 0; mt < 4; mt++) {
                uint32_t ad = __cvta_generic_to_shared(&a_s[aoff + (a_rowt[mt] + arow) * AW + sub * AROWB + acblk * 16]);
                asm volatile("ldmatrix.sync.aligned.m8n8.x4.shared::cta.b16 {%0,%1,%2,%3}, [%4];"
                             : "=r"(af[mt][0]), "=r"(af[mt][1]), "=r"(af[mt][2]), "=r"(af[mt][3]) : "r"(ad));
            }
#pragma unroll
            for (int n = 0; n < 8; n++) {
                uint32_t bd = __cvta_generic_to_shared(&b_s[boff + (b_col[n] + nrow) * BW_ + sub * BROWB + bsub * 16]);
                asm volatile("ldmatrix.sync.aligned.m8n8.x4.shared::cta.b16 {%0,%1,%2,%3}, [%4];"
...
void run(int sz){
    ...
    alignas(64) CUtensorMap mapA,mapB; mk(&mapA,dA,KAb,M,AW,BM); mk(&mapB,dB,KBb,N,BW_,BN);
    ...
    printf("matmul_sp_wide (WK=2 wide-TMA): %dx%dx%d  %.3f ms  %.1f GFLOP/s  %s (out[0]=%.0f exp %d)\n",

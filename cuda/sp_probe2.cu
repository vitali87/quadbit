...
// PROBE 2: all-ones A (every stored=1.0). B[k] depends only on (k mod 4):
//   f(0)=1.0(0x2) f(1)=1.5(0x3) f(2)=2.0(0x4) f(3)=3.0(0x5).
// out = (#groups) * (f(p0)+f(p1)) for the metadata-selected pair {p0,p1}. #groups =
// Klog/4. Read out[0] to identify what metadata 0x44444444 selects (assuming B-routing).
//   {0,1}->2.5  {0,2}->3.0  {0,3}->4.0  {1,2}->3.5  {1,3}->4.5  {2,3}->5.0  (x #groups)
void run(int sz) {
    ...
    // B[j][k] = f(k mod 4); byte p (k=2p,2p+1): p even -> f(0)|f(1)<<4, p odd -> f(2)|f(3)<<4
    uint8_t *hB = (uint8_t *)malloc((size_t)N * KBb);
    for (int j = 0; j < N; j++)
        for (int p = 0; p < KBb; p++)
            hB[(size_t)j * KBb + p] = (p & 1) ? (0x4 | (0x5 << 4)) : (0x2 | (0x3 << 4));
    ...
    int g = Klog / 4;
    printf("sp_probe2 (metadata-meaning): Klog=%d groups=%d  out[0]=%.1f out[1]=%.1f  pairsum=%.3f\n",
           Klog, g, hC[0], hC[1], hC[0] / g);
    printf("  candidates per-group: {0,1}=2.5 {0,2}=3.0 {0,3}=4.0 {1,2}=3.5 {1,3}=4.5 {2,3}=5.0\n");

...
#pragma unroll
    ...
    for (int ls = 0; ls < lsteps; ls++) {
        ...
        acc[0][0]+=(float)a_s[aoff+wtid]+(float)b_s[boff+wtid];
...
void run(int sz, bool ref){
    ...
    alignas(64) CUtensorMap mapA,mapB; mk(&mapA,dA,KAb,M,AW,BM,CU_TENSOR_MAP_SWIZZLE_NONE); mk(&mapB,dB,KBb,N,BW_,BN,CU_TENSOR_MAP_SWIZZLE_NONE);

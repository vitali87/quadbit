...
#pragma unroll
    ...
    // PERF PROBE: ldmatrix hoisted OUT of the loop (done once). Output is WRONG; this
    // isolates TMA-bandwidth + sync + mma from per-step LDSM cost. If GFLOP/s jumps
    // toward the 3626k mma ceiling, LDSM is the bottleneck; if it stays ~1626k, TMA is.
    uint32_t af[4][4], bf[8][4];
#pragma unroll
    for (int mt = 0; mt < 4; mt++) {
        uint32_t ad = __cvta_generic_to_shared(&a_s[(a_rowt[mt] + arow) * AROWB + acblk * 16]);
        asm volatile("ldmatrix.sync.aligned.m8n8.x4.shared::cta.b16 {%0,%1,%2,%3}, [%4];"
                     : "=r"(af[mt][0]), "=r"(af[mt][1]), "=r"(af[mt][2]), "=r"(af[mt][3]) : "r"(ad));
    }
#pragma unroll
    for (int n = 0; n < 8; n++) {
        uint32_t bd = __cvta_generic_to_shared(&b_s[(b_col[n] + nrow) * BROWB + bsub * 16]);
        asm volatile("ldmatrix.sync.aligned.m8n8.x4.shared::cta.b16 {%0,%1,%2,%3}, [%4];"
...
void run(int sz) {
    ...
    printf("matmul_sp_ldprobe (NO per-step LDSM, WRONG output): %dx%dx%d  %.3f ms  %.1f GFLOP/s  %s (out[0]=%.0f exp %d)\n",

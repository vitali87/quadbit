// Raw-PTX track, warp-spec milestone, step 1: replace the 989k kernel's cp.async
// staging (all 128 threads copy 16-byte chunks inside the mainloop) with TMA bulk
// loads (one elected thread issues a bulk copy; mbarrier tracks completion). Same
// proven 64x128 / 2x8 / ldmatrix consumer inner, all 4 warps stay consumers. Tests
// whether cheaper TMA issue beats cp.async before adding warp-specialization.
// Multi-stage ring buffer; host builds u8 CUtensorMaps for A and B.
...
void run(int sz) {
    ...
    printf("matmul_fp4_tma: %dx%dx%d  %.3f ms  %.1f GFLOP/s  %s (%d wrong)\n",

...
void runMNK(int M,int N,int Klog){
    ...
    for(int i=0;i<it;i++) matmul_sp<<<grid,block,SMEM>>>(mapA,mapB,dC,M,N,Klog);
    cudaEventRecord(e);cudaEventSynchronize(e);float ms=0;cudaEventElapsedTime(&ms,s,e);ms/=it;
    double bbytes=(double)N*KBb;  // B read once (1 row-block) dominant for thin M
    printf("thinM M=%-5d N=%d K=%d: %.4f ms  %.0f GFLOP/s  Bread=%.1fGB/s\n",M,N,Klog,ms,2.0*M*N*Klog/(ms/1e3)/1e9, bbytes/(ms/1e3)/1e9);
    cudaFree(dA);cudaFree(dB);cudaFree(dC);
}

int main(){ runMNK(256,8192,8192); runMNK(512,8192,8192); runMNK(1024,8192,8192); runMNK(2048,8192,8192); runMNK(8192,8192,8192); return 0; }

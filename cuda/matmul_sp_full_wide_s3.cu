// FULLY DEPLOYABLE + WIDE-SWIZZLE: arbitrary 2:4 metadata + real ue4m3 scales, with the
// wide-TMA+swizzle load path that broke the unit kernel's false roofline (2012->2731k).
// A/B loaded WK=2 k128-slices per TMA (wide, swizzled: A 64B / B 128B) to hit L2 BW;
// scaleA/scaleB/meta staged per-step (2 per WK=2 chunk). 2 mma rounds/chunk. STAGES=2.
#include <cstring>
#include <cstdio>
#include <cstdint>
#include <cuda.h>
...
#pragma unroll
    ...
    int grA[4], grM[4], gcB[8];
#pragma unroll
    for (int mt = 0; mt < 4; mt++) { grA[mt] = block_row + a_rowt[mt] + ra_local; grM[mt] = block_row + a_rowt[mt] + mma_row; }
#pragma unroll
    for (int n = 0; n < 8; n++) gcB[n] = block_col + b_col[n] + cb_local;
...
#pragma unroll
            for (int mt = 0; mt < 4; mt++) {
                sav[mt] = a_valid ? __ldg(sA32 + (size_t)(step * M + grA[mt])) : 0x38383838u;
                ev[mt] = __ldg(meta + (size_t)(step * M + grM[mt]) * 2 + Hh);
            }
#pragma unroll
            for (int n = 0; n < 8; n++) sbv[n] = b_valid ? __ldg(sB32 + (size_t)(step * N + gcB[n])) : 0x38383838u;
...
void run(int sz, bool ref) {
    ...
    if(ref){int wrong=0;float maxrel=0;
        ...
        printf("matmul_sp_full_wide_s3 VERIFY %dx%dx%d: %s (maxrel %.4f)\n",sz,sz,sz,wrong==0?"PASS":"FAIL",maxrel);
    } else printf("matmul_sp_full_wide_s3 %dx%dx%d: %.3f ms  %.1f GFLOP/s\n",sz,sz,sz,ms,gf);
...
int main(){ run(512,true); run(1024,true); run(2048,false); run(8192,false); return 0; }

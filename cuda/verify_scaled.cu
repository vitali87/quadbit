...
#pragma unroll
    ...
    uint16_t z = 0;
    // per-lane scale-register holders: A lane holds row=(lane>>2)+8*(lane&1) when
    // (lane&3)<2; B lane holds col=lane>>2 when (lane&3)==0 (derived in scale_probe*).
...
#pragma unroll
        for (int ks = 0; ks < BK / 64; ks++) {
            int kb = ks * 32;
            int kbi = step * (BK / 32) + ks * 2;  // global K-block of this mma's kb0
...
void run(int sz) {
    ...
    // ue8m0 scales near 127 (2^0): exponents 125..129 -> 2^-2..2^+2
    for (size_t i = 0; i < (size_t)M * Ksf; i++) hSFA[i] = 125 + (rnd() % 5);
    for (size_t i = 0; i < (size_t)N * Ksf; i++) hSFB[i] = 125 + (rnd() % 5);
    ...
    matmul_fp4<<<grid, block>>>(mapA, mapB, dC, N, Kf, dSFA, dSFB, Ksf);
    ...
    // host reference with block scales: value = decode(nibble) * 2^(ue8m0-127)
    int wrong = 0;
    float maxrel = 0.f;
    for (int m = 0; m < M && wrong < 10; m++)
        for (int n = 0; n < N; n++) {
            ...
            for (int k = 0; k < Kf; k++) {
                ...
                float av = decode_fp4(k & 1 ? ba >> 4 : ba & 0xf);
                float bv = decode_fp4(k & 1 ? bb >> 4 : bb & 0xf);
                float sa = ldexpf(1.f, (int)hSFA[(size_t)m * Ksf + k / 32] - 127);
                float sb = ldexpf(1.f, (int)hSFB[(size_t)n * Ksf + k / 32] - 127);
                ref += (av * sa) * (bv * sb);
            }
            ...
            if (rel > 1e-2f) {
                if (wrong < 5) printf("  mismatch [%d][%d]: got %.3f ref %.3f\n", m, n, got, ref);
                wrong++;
            }
        }
    printf("verify_scaled (random values + random ue8m0 scales): %dx%dx%d  %s (%d wrong, maxrel %.4f)\n",

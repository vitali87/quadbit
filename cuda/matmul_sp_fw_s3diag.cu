...
#pragma unroll
            for (int mt = 0; mt < 4; mt++) { sav[mt]=0x38383838u; ev[mt]=0x44444444u; }
#pragma unroll
            for (int n = 0; n < 8; n++) sbv[n]=0x38383838u;
...
void run(int sz, bool ref) {
    ...
    if(ref){int wrong=0;float maxrel=0;
        ...
        printf("matmul_sp_fw_s3diag VERIFY %dx%dx%d: %s (maxrel %.4f)\n",sz,sz,sz,wrong==0?"PASS":"FAIL",maxrel);
    } else printf("matmul_sp_fw_s3diag %dx%dx%d: %.3f ms  %.1f GFLOP/s\n",sz,sz,sz,ms,gf);

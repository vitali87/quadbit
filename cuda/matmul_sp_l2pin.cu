...
void run(int sz) {
    ...
    size_t Ab = (size_t)M * KAb, Bb = (size_t)N * KBb;
    // A and B contiguous in ONE allocation so a single L2 access-policy window covers both
    uint8_t *dAB; cudaMalloc(&dAB, Ab + Bb);
    uint8_t *dA = dAB, *dB = dAB + Ab;
    __nv_bfloat16 *dC; cudaMalloc(&dC, (size_t)M * N * sizeof(__nv_bfloat16));
    cudaMemset(dA, 0x22, Ab); cudaMemset(dB, 0x22, Bb);

    int l2 = 0, maxPersist = 0, maxWin = 0;
    ...
    size_t want = Ab + Bb;
    size_t carve = want < (size_t)maxPersist ? want : (size_t)maxPersist;
    cudaDeviceSetLimit(cudaLimitPersistingL2CacheSize, carve);
    cudaStream_t st; cudaStreamCreate(&st);
    size_t win = want < (size_t)maxWin ? want : (size_t)maxWin;
    cudaStreamAttrValue av; memset(&av, 0, sizeof(av));
    av.accessPolicyWindow.base_ptr = dAB;
    ...
    for (int i = 0; i < 8; i++) matmul_sp<<<grid, block, SMEM, st>>>(mapA, mapB, dC, M, N, Klog);
    ...
    cudaEventRecord(s, st); for (int i = 0; i < it; i++) matmul_sp<<<grid, block, SMEM, st>>>(mapA, mapB, dC, M, N, Klog);
    cudaEventRecord(e, st); cudaEventSynchronize(e); float ms = 0; cudaEventElapsedTime(&ms, s, e); ms /= it;
    ...
    printf("matmul_sp_l2pin %dx%dx%d: L2=%dMB maxPersist=%dMB win=%.1fMB(AB=%.1f) %.3f ms %.1f GFLOP/s %s\n",
           M, N, Klog, l2>>20, maxPersist>>20, win/1048576.0, want/1048576.0, ms, gf, wrong==0?"PASS":"FAIL");
    free(hC); cudaFree(dAB); cudaFree(dC); cudaStreamDestroy(st);
}

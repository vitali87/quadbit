...
void run(int sz) {
    ...
    // PROBE 1: B constant over k (B[j][k]=Vj for all k). Then out[i][j] = Vj * sum_s
    // A_stored[i][s], INDEPENDENT of metadata + B-k-routing -> isolates the A-stored
    // fragment + output layout + scale. Vj nibble = j&7 (avoid sign for simplicity).
    for (int j = 0; j < N; j++) {
        uint8_t vn = (uint8_t)(j & 7);  // e2m1 nibble 0..7 (nonneg values)
        for (int p = 0; p < KBb; p++) hB[(size_t)j * KBb + p] = vn | (vn << 4);
    }
    ...
    // ref: out[i][j] = decode(Vj) * sum over all stored A nibbles of row i
    int wrong = 0; float maxrel = 0.f;
    for (int i = 0; i < M && wrong < 8; i++) {
        float asum = 0.f;
        for (int p = 0; p < KAb; p++) {
            uint8_t b = hAc[(size_t)i * KAb + p];
            asum += decode_fp4(b & 0xf) + decode_fp4(b >> 4);
        }
        for (int j = 0; j < N; j++) {
            float ref = decode_fp4((uint8_t)(j & 7)) * asum;
            float got = hC[(size_t)i * N + j];
            float rel = fabsf(got - ref) / (fabsf(ref) + 1.f);
            if (rel > maxrel) maxrel = rel;
            if (rel > 1e-2f) { if (wrong < 4) printf("  [%d][%d] got %.2f ref %.2f (asum=%.1f Vj=%.1f)\n", i, j, got, ref, asum, decode_fp4(j & 7)); wrong++; }
        }
    }
    printf("sp_probe1 (B const over k -> isolates A-stored/output/scale): %dx%dx%d  %s (%d wrong, maxrel %.4f)\n",

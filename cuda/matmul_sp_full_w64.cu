...
#pragma unroll
        for (int sub = 0; sub < WK; sub++) {
            ...
            uint32_t sav[4], sbv[4], ev[4];
#pragma unroll
            for (int mt = 0; mt < 4; mt++) { sav[mt] = a_valid ? scA[a_sidx[mt]] : 0x38383838u; ev[mt] = mtA[m_sidx[mt]]; }
#pragma unroll
            for (int n = 0; n < 4; n++) sbv[n] = b_valid ? scB[b_sidx[n]] : 0x38383838u;
...
#pragma unroll
                for (int n = 0; n < 4; n++) {
                    int idx = mt * 4 + n; float d0, d1, d2, d3;
                    asm volatile(
...
void run(int sz, bool ref) {
    ...
    if(ref){int wrong=0;float maxrel=0;
        ...
        printf("matmul_sp_full_w64 VERIFY %dx%dx%d: %s (maxrel %.4f)\n",sz,sz,sz,wrong==0?"PASS":"FAIL",maxrel);
    } else printf("matmul_sp_full_w64 %dx%dx%d: %.3f ms  %.1f GFLOP/s\n",sz,sz,sz,ms,gf);

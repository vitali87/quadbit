...
#pragma unroll
    ...
    for (int step = 0; step < ksteps; step++) {
        ...
        // ILP: load ALL fragments for both k-substeps, THEN issue all 64 mma back-to-back
        // (no ldmatrix stalls interleaved in the mma stream -> higher OMMA density).
...
#pragma unroll
            for (int mt = 0; mt < 4; mt++) {
                ...
                asm volatile("ldmatrix.sync.aligned.m8n8.x4.shared::cta.b16 {%0,%1,%2,%3}, [%4];"
                             : "=r"(af[ks][mt][0]), "=r"(af[ks][mt][1]), "=r"(af[ks][mt][2]), "=r"(af[ks][mt][3]) : "r"(ad));
            }
#pragma unroll
            for (int n = 0; n < 8; n++) {
...
#pragma unroll
                for (int n = 0; n < 8; n++) {
                    ...
                    asm volatile(
                        ...
                        : "=f"(d0), "=f"(d1), "=f"(d2), "=f"(d3)
                        : "r"(af[ks][mt][0]), "r"(af[ks][mt][1]), "r"(af[ks][mt][2]), "r"(af[ks][mt][3]), "r"(bf[ks][n][0]), "r"(bf[ks][n][1]),
...
void run(int sz) {
    ...
    printf("matmul_fp4_ilp: %dx%dx%d  %.3f ms  %.1f GFLOP/s  %s (%d wrong)\n",

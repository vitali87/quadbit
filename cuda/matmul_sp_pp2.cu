// Raw-PTX track: 2:4-sparse FP4 matmul, K-unrolled by 2. Sparse mma.sp halves the mma
// vs dense but keeps full per-step overhead (mbarrier wait + 12 LDSM + bar.sync +
// producer branch), so it runs overhead-bound (~52% of mma peak). This variant consumes
// TWO K-slices per loop iteration with ONE bar.sync + ONE producer branch per pair,
// halving the sync/branch overhead per mma. STAGES=4 (two buffers in compute, two ahead).
// Frags reused across the two sub-steps so register pressure stays put. All-ones => Klog/2.
...
#pragma unroll
    ...
    // one mma.sp K-slice into acc from buffer s (frags reused across sub-steps)
    auto slice = [&](int st) {
        int s = st % STAGES;
        uint32_t par = (st / STAGES) & 1;
        asm volatile("{\n\t.reg .pred p;\n\tWAIT:\n\t"
...
#pragma unroll
    ...
    for (int step = 0; step < ksteps; step += 2) {
        ...
        asm volatile("bar.sync %0, 128;" ::"r"(sync_id));
        if (wtid == 0) {
            int n0 = step + STAGES, n1 = step + 1 + STAGES;
            if (n0 < ksteps) issue(step % STAGES, n0);
            if (n1 < ksteps) issue((step + 1) % STAGES, n1);
        }
    }
...
void run(int sz) {
    ...
    for (int i = 0; i < 5; i++) matmul_sp<<<grid, block, SMEM>>>(mapA, mapB, dC, N, Klog);
    ...
    int iters = 20;
    cudaEventRecord(s);
    for (int i = 0; i < iters; i++) matmul_sp<<<grid, block, SMEM>>>(mapA, mapB, dC, N, Klog);
    ...
    printf("matmul_sp_pp2 (2:4 sparse, K-unroll2): %dx%dx%d  %.3f ms  %.1f GFLOP/s  %s (out[0]=%.0f exp %d)\n",

...
#pragma unroll
    ...
    for (int step = 0; step < ksteps; step++) {
        ...
        // MEM-ONLY PROBE: no LDSM, no mma. Just touch one byte of each buffer so the
        // loads aren't pointless; loop time = pure TMA pipeline time. If GFLOP/s == kernel
        // (~1626k), the kernel is fully memory-bound; if far higher, it is not.
...
void run(int sz) {
    ...
    printf("matmul_sp_memonly (TMA only, no compute): %dx%dx%d  %.3f ms  %.1f GFLOP/s  %s (out[0]=%.0f exp %d)\n",

...
// ---- 2:4-NVFP4 weight PACKING (the missing "producer") + end-to-end round-trip ----
// Given dense weights/activations, produce exactly what the kernel consumes:
//   - 2:4 prune A by MAGNITUDE (keep the 2 largest of every 4 b16-pairs per group)
//   - emit metadata (kept pair-indices), compressed A (kept values in slot order)
//   - per-block ue4m3 scales, step-major scaleA/scaleB/meta layouts
// Dense values are generated as exact fp4 x ue4m3 so the round-trip error is bf16-only,
// isolating PACKING correctness. Reference = dense matmul of the pruned+dequant values.
//   fp4 codes: 0=0 1=.5 2=1 3=1.5 4=2 5=3 6=4 7=6 (|.|); pair magnitude = |v0|+|v1|.

int main() {
    auto go = [](int sz) {
        int M = sz, N = sz, Klog = sz, KAb = Klog / 4, KBb = Klog / 2, ksteps = Klog / BKL;
        uint8_t *hAc = (uint8_t *)malloc((size_t)M * KAb);
        uint8_t *hB = (uint8_t *)malloc((size_t)N * KBb);
        uint8_t *hScA = (uint8_t *)malloc((size_t)ksteps * M * 4);
        uint8_t *hScB = (uint8_t *)malloc((size_t)ksteps * N * 4);
        uint32_t *hMeta = (uint32_t *)calloc((size_t)ksteps * M * 2, 4);
        uint8_t *idxA = (uint8_t *)malloc((size_t)M * ksteps * 16 * 2);  // kept pair-indices for ref
        uint32_t st = 0x33u; auto rnd = [&]() { st ^= st << 13; st ^= st >> 17; st ^= st << 5; return st; };
        auto mag = [](uint8_t c) { float v = dfp4(c); return v < 0 ? -v : v; };

        // scales: random ue4m3 (sign 0, exp 6..8)
        for (size_t i = 0; i < (size_t)ksteps * M * 4; i++) hScA[i] = ((6 + (rnd() % 3)) << 3) | (rnd() & 7);
        for (size_t i = 0; i < (size_t)ksteps * N * 4; i++) hScB[i] = ((6 + (rnd() % 3)) << 3) | (rnd() & 7);
        // B: random fp4 codes (the dense, already block-scaled form)
        for (size_t i = 0; i < (size_t)N * KBb; i++) hB[i] = (uint8_t)rnd();

        // A: per (row, step, group of 4 pairs) generate 4 pairs, keep 2 by magnitude
        for (int i = 0; i < M; i++)
            for (int step = 0; step < ksteps; step++)
                for (int g = 0; g < 16; g++) {
                    uint8_t plo[4], phi[4]; float pm[4];
                    for (int p = 0; p < 4; p++) { plo[p] = rnd() & 0xf; phi[p] = rnd() & 0xf; pm[p] = mag(plo[p]) + mag(phi[p]); }
                    // pick the 2 largest-magnitude pair indices (idx0 < idx1)
                    ...
                    // compressed A: slot0 = pair i0, slot1 = pair i1
                    int cs0 = step * 32 + g * 2 + 0, cs1 = step * 32 + g * 2 + 1;
                    hAc[(size_t)i * KAb + cs0] = plo[i0] | (phi[i0] << 4);
                    hAc[(size_t)i * KAb + cs1] = plo[i1] | (phi[i1] << 4);
                }
        ...
        // dense reference over kept (compressed) positions of the pruned+dequant weights
            ...
            for (int j = 0; j < N; j++) {
                ...
                for (int step = 0; step < ksteps; step++)
                    for (int cs = 0; cs < 32; cs++) {
                        ...
                        int pidx = idxA[(((size_t)i * ksteps + step) * 16 + g) * 2 + slot];
                        int pair = step * 64 + g * 4 + pidx, gKlo = 2 * pair, gKhi = 2 * pair + 1;
                        float sA = due4m3(hScA[((size_t)step * M + i) * 4 + cs / 8]);
                        uint8_t ab = hAc[(size_t)i * KAb + step * 32 + cs];
                        uint8_t bl = hB[(size_t)j * KBb + gKlo / 2], bh = hB[(size_t)j * KBb + gKhi / 2];
                        float Bl = dfp4((gKlo & 1) ? bl >> 4 : bl & 0xf) * due4m3(hScB[((size_t)step * N + j) * 4 + (gKlo - step * 128) / 32]);
                        float Bh = dfp4((gKhi & 1) ? bh >> 4 : bh & 0xf) * due4m3(hScB[((size_t)step * N + j) * 4 + (gKhi - step * 128) / 32]);
                        ref += dfp4(ab & 0xf) * sA * Bl + dfp4(ab >> 4) * sA * Bh;
                    }
        ...
        printf("pack_verify (dense -> 2:4-by-magnitude + NVFP4 pack -> kernel, %dx%dx%d): %s (maxrel %.5f)\n",
               sz, sz, sz, wrong == 0 ? "PASS" : "FAIL", maxrel);
        free(hAc); free(hB); free(hScA); free(hScB); free(hMeta); free(idxA); free(hC);
        cudaFree(dA); cudaFree(dB); cudaFree(dScA); cudaFree(dScB); cudaFree(dMeta); cudaFree(dC);
    };
    go(512); go(1024); go(2048);
    return 0;
}

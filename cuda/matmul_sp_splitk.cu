...
matmul_sp(const __grid_constant__ CUtensorMap mapA, const __grid_constant__ CUtensorMap mapB,
          float *C, int M, int N, int Klog, int KSPLIT) {
    ...
    int lsteps = Klog / (128 * WK);
    int kz = blockIdx.z, per = lsteps / KSPLIT, ls0 = kz * per, ls1 = ls0 + per;   // wide load-steps
...
#pragma unroll
        for (int s = 0; s < STAGES; s++)
            if (ls0 + s < ls1) issue(s, ls0 + s);

    for (int ls = ls0; ls < ls1; ls++) {
        int ll = ls - ls0; int s = ll % STAGES; uint32_t par = (ll / STAGES) & 1;
...
#pragma unroll
        for (int n = 0; n < 8; n++) {
            ...
            atomicAdd(&C[gr * N + gc], acc[idx][0]); atomicAdd(&C[gr * N + gc + 1], acc[idx][1]);
            atomicAdd(&C[(gr + 8) * N + gc], acc[idx][2]); atomicAdd(&C[(gr + 8) * N + gc + 1], acc[idx][3]);
        }
}
...
void runMNK(int M,int N,int Klog,int KSPLIT){
    int KAb=Klog/4,KBb=Klog/2; uint8_t*dA,*dB; float*dC;
    ...
    dim3 grid(N/BN,M/(2*BM),KSPLIT),block(256);
    for(int i=0;i<3;i++){cudaMemset(dC,0,(size_t)M*N*4); matmul_sp<<<grid,block,SMEM>>>(mapA,mapB,dC,M,N,Klog,KSPLIT);}
    if(cudaDeviceSynchronize()!=cudaSuccess){printf("CUDA error: %s\n",cudaGetErrorString(cudaGetLastError()));return;}
    cudaEvent_t s,e;cudaEventCreate(&s);cudaEventCreate(&e);int it=30;cudaEventRecord(s);
    for(int i=0;i<it;i++){cudaMemset(dC,0,(size_t)M*N*4); matmul_sp<<<grid,block,SMEM>>>(mapA,mapB,dC,M,N,Klog,KSPLIT);}
    ...
    printf("splitK M=%-5d N=%d K=%d KSPLIT=%d: %.4f ms  %.0f GFLOP/s  %s (CTAs=%d)\n",
           M,N,Klog,KSPLIT,ms,2.0*M*N*Klog/(ms/1e3)/1e9,wrong==0?"PASS":"FAIL",(N/BN)*(M/(2*BM))*KSPLIT);
    free(hC);cudaFree(dA);cudaFree(dB);cudaFree(dC);
}
int main(){
    runMNK(256,8192,8192,1); runMNK(256,8192,8192,4); runMNK(256,8192,8192,8);
    runMNK(512,8192,8192,1); runMNK(512,8192,8192,4);
    return 0;
}

...
fn matmul_fp4_ldmpipe<A: Scalar, B: Scalar, CD: Numeric, S: Scalar, NA: Size, NB: Size, NC: Size>(
    a: &TensorMap<A, Tiled>,
    b: &TensorMap<B, Tiled>,
    ...
    let mut a_tile = SharedMemory::<A>::new_aligned(2 * BM * BKH, 128usize);
    let mut b_tile = SharedMemory::<B>::new_aligned(2 * BN * BKH, 128usize);
    ...
    let abuf = comptime!(BM * BKH);
    let bbuf = comptime!(BN * BKH);
    let ab_bytes = select(UNIT_POS_X == 0, comptime!((BM * BKH + BN * BKH) as u32), 0u32);
    // double-buffered TMA (NO warp specialization): elected thread issues the TMA a
    // step ahead while all 4 warps stay on ldmatrix+MMA, so the wait does not block.
    ...
    for step in 0..k_steps {
        ...
        // prefetch step+2 into the buffer this stage just freed, and re-arm
        let next = step + 2;
        if next < k_steps {
...
fn verify<R: Runtime>(device: &R::Device) {
    ...
    unsafe {
        matmul_fp4_ldmpipe::launch_unchecked::<AB, AB, f32, S, R>(
            &client,
...
fn run_size<R: Runtime>(device: &R::Device, sz: usize) {
    ...
    for _ in 0..20 {
        let (_, profile) = client.profile(|| launch(out_h.clone()), "matmul_fp4_ldmpipe").unwrap();

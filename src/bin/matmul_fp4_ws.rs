//! Warp-specialized block-scaled FP4 (MXFP4) matmul on SM120, built to test
//! whether dedicating warps to staging (producer/consumer) can beat the
//! register-resident 2x8 baseline (`matmul_fp4_fed`). It does NOT, and this
//! kernel is the empirical evidence.
//!
//! Layout: 6 warps/block. Warps 0-3 are CONSUMERS running the exact proven 2x2
//! grid / 2x8 warp tile (16 accumulator chains each). Warps 4-5 are PRODUCERS
//! that own the global->shared staging of A, B and the UE8M0 scales. Single
//! buffered: producers fill, barrier, consumers compute, barrier.
//!
//! Why it cannot win on SM120: registers are allocated uniformly per kernel and
//! capped at 255/thread (hardware max), and the SM register file (65536) is
//! already saturated by the baseline at 8 warps x 255 = 65280. A specialized
//! block (consumers at 255 + a producer warp) fits exactly ONE block/SM whether
//! the producer holds 255 or 24 registers, so `setmaxnreg` cannot add a second
//! block. The best a specialized block can do is ~7 compute warps/SM; this
//! concrete 4-consumer-warp config gives 4, both below the baseline's 8. Fewer
//! compute warps => fewer in-flight OMMA chains => less latency hiding => slower.
//! All-ones FP4 with unit scales, so each output must equal K.
...
const SF: usize = 2;
const WARPS_M: usize = 2; // consumer warps across the block in m
const WARPS_N: usize = 2; // consumer warps across the block in n
const WM: usize = 2;
const WN: usize = 8;
...
const PRODUCER_WARPS: usize = 2;
const N_THREADS: u32 = ((CONSUMER_WARPS + PRODUCER_WARPS) * 32) as u32; // 192
const PROD_THREADS: usize = PRODUCER_WARPS * 32; // 64

#[cube(launch_unchecked)]
fn matmul_fp4_ws<A: Scalar, B: Scalar, CD: Numeric, S: Scalar, NA: Size, NB: Size, NC: Size>(
    ...
    for step in 0..k_steps {
        ...
        // PRODUCERS (warps 4-5, 64 threads) own all global->shared staging
        if warp >= CONSUMER_WARPS {
            let ptid = tid - CONSUMER_WARPS * 32;
            #[unroll]
            for i in 0..(BM * VPR) / PROD_THREADS {
                let s = ptid + i * PROD_THREADS;
                let r = s / VPR;
                let vc = s % VPR;
                a_tile[s] = a[(block_row + r) * global_vpr + k_vec + vc];
            }
            #[unroll]
            for i in 0..(BN * VPR) / PROD_THREADS {
                let s = ptid + i * PROD_THREADS;
                let r = s / VPR;
                let vc = s % VPR;
        ...
        // CONSUMERS (warps 0-3) walk the staged tile MMA-K at a time
        if warp < CONSUMER_WARPS {
            #[unroll]
            for ks in 0..KSUB {
                let k_off = ks * MMA_K;
...
fn verify<R: Runtime>(device: &R::Device) {
    ...
    unsafe {
        matmul_fp4_ws::launch_unchecked::<AB, AB, f32, S, R>(
            &client,
...
fn run<R: Runtime>(device: &R::Device) {
    ...
    for _ in 0..20 {
        let (_, profile) = client.profile(|| launch(out_h.clone()), "matmul_fp4_ws").unwrap();
    ...
    println!("runtime: {:?}", R::name(&client));
    println!("shape: {m}x{n}x{k}  (warp-specialized: {CONSUMER_WARPS} consumer + {PRODUCER_WARPS} producer warps)");

//! Milestone 3 toward closing the 3x gap to CUTLASS: TMA staging (milestone 1) PLUS
//! FP4 `ldmatrix` operand loads, which replace the per-substep `position_of_nth` +
//! LDS fragment gathers that milestone 2 pinned as the bottleneck. Uses the plain
//! `m8n8.x{factor}.b16` ldmatrix over the bit-packed buffer (the patched cubecl-cpp
//! emits it), NON-transposed, row-major — NO swizzle, the layout matches
//! `mma.sync.mxf4` exactly (derived in `src/bin/ldm_probe.rs`: A and B both MATCH).
//! Per-lane addressing: A m8n8.x4 with `arow=(lane/8%2)*8+lane%8`, `cblk=(lane/8)/2`;
//! B m8n8.x2 with `nrow=lane%8`, `kblk=(lane/8)%2`. A/B are TMA-staged into 128-byte
//! aligned SCALAR shared (ldmatrix needs a `Slice<A>`); scales stay on the manual
//! gather. Verified against the same f32 reference as fed.
...
fn matmul_fp4_ldm2<A: Scalar, B: Scalar, CD: Numeric, S: Scalar, NA: Size, NB: Size, NC: Size>(
    a: &TensorMap<A, Tiled>,
    b: &TensorMap<B, Tiled>,
    ...
    // TMA needs 128-byte-aligned shared; SCALAR so ldmatrix gets a Slice<A>. Row
    // stride = BKH (64) bytes; the staged tile is BM/BN rows x BKH bytes.
    let mut a_tile = SharedMemory::<A>::new_aligned(BM * BKH, 128usize);
    let mut b_tile = SharedMemory::<B>::new_aligned(BN * BKH, 128usize);
    ...
    // per-lane ldmatrix tile coordinates (lane-constant; derived in ldm_probe)
    let la_arow = ((lane_id / 8) % 2) as usize * 8 + (lane_id % 8) as usize; // A row 0..15
    let la_cblk = ((lane_id / 8) / 2) as usize; // A 16-byte col half 0/1
    let lb_nrow = (lane_id % 8) as usize; // B n row 0..7
    let lb_kblk = ((lane_id / 8) % 2) as usize; // B 16-byte k half 0/1
    ...
    let barrier = Barrier::shared(N_THREADS, UNIT_POS_X == 0);
    sync_async_proxy_shared();

    for step in 0..k_steps {
        let kcol = (step * BKH) as i32; // packed-column origin of this k-step tile

        // TMA: one bulk load each for the A and B tiles into aligned shared.
        if tid == 0 {
            barrier.tma_load_2d(a, &mut a_tile.to_slice_mut(), block_row as i32, kcol);
            barrier.tma_load_2d(b, &mut b_tile.to_slice_mut(), block_col as i32, kcol);
        }
        let expected = select(UNIT_POS_X == 0, comptime!((BM * BKH + BN * BKH) as u32), 0u32);
        ...
        barrier.wait(token); // A and B fully staged
        ...
        for ks in 0..KSUB {
            ...
            let kb = k_off / 2; // this substep's byte column offset (ks*32)
            let a0s = (a_row0 + la_arow) * BKH + kb + la_cblk * 16;
            let asl0 = a_tile.slice(a0s, a0s + 16);
            def.load_matrix_inplace::<A, NA>(&asl0, &mut a0, MatrixIdent::A, vector_count_a, false);
            let a1s = (a_row1 + la_arow) * BKH + kb + la_cblk * 16;
            let asl1 = a_tile.slice(a1s, a1s + 16);
            def.load_matrix_inplace::<A, NA>(&asl1, &mut a1, MatrixIdent::A, vector_count_a, false);
            #[unroll]
            for i in 0..scales_count {
                sa0[i] = sa_tile[(a_row0 + scales_idx_a) * bps + sk + i];
                sa1[i] = sa_tile[(a_row1 + scales_idx_a) * bps + sk + i];
            }
            let b0s = (b_col0 + lb_nrow) * BKH + kb + lb_kblk * 16;
            let bsl0 = b_tile.slice(b0s, b0s + 16);
            def.load_matrix_inplace::<B, NB>(&bsl0, &mut b0, MatrixIdent::B, vector_count_b, false);
            let b1s = (b_col1 + lb_nrow) * BKH + kb + lb_kblk * 16;
            let bsl1 = b_tile.slice(b1s, b1s + 16);
            def.load_matrix_inplace::<B, NB>(&bsl1, &mut b1, MatrixIdent::B, vector_count_b, false);
            let b2s = (b_col2 + lb_nrow) * BKH + kb + lb_kblk * 16;
            let bsl2 = b_tile.slice(b2s, b2s + 16);
            def.load_matrix_inplace::<B, NB>(&bsl2, &mut b2, MatrixIdent::B, vector_count_b, false);
            let b3s = (b_col3 + lb_nrow) * BKH + kb + lb_kblk * 16;
            let bsl3 = b_tile.slice(b3s, b3s + 16);
            def.load_matrix_inplace::<B, NB>(&bsl3, &mut b3, MatrixIdent::B, vector_count_b, false);
            let b4s = (b_col4 + lb_nrow) * BKH + kb + lb_kblk * 16;
            let bsl4 = b_tile.slice(b4s, b4s + 16);
            def.load_matrix_inplace::<B, NB>(&bsl4, &mut b4, MatrixIdent::B, vector_count_b, false);
            let b5s = (b_col5 + lb_nrow) * BKH + kb + lb_kblk * 16;
            let bsl5 = b_tile.slice(b5s, b5s + 16);
            def.load_matrix_inplace::<B, NB>(&bsl5, &mut b5, MatrixIdent::B, vector_count_b, false);
            let b6s = (b_col6 + lb_nrow) * BKH + kb + lb_kblk * 16;
            let bsl6 = b_tile.slice(b6s, b6s + 16);
            def.load_matrix_inplace::<B, NB>(&bsl6, &mut b6, MatrixIdent::B, vector_count_b, false);
            let b7s = (b_col7 + lb_nrow) * BKH + kb + lb_kblk * 16;
            let bsl7 = b_tile.slice(b7s, b7s + 16);
            def.load_matrix_inplace::<B, NB>(&bsl7, &mut b7, MatrixIdent::B, vector_count_b, false);
            #[unroll]
            for i in 0..scales_count {
        ...
        sync_cube(); // before next step's TMA overwrites the tiles
    }
...
fn verify<R: Runtime>(device: &R::Device) {
    ...
    unsafe {
        matmul_fp4_ldm2::launch_unchecked::<AB, AB, f32, S, R>(
            &client,
...
fn run_size<R: Runtime>(device: &R::Device, sz: usize) {
    ...
    for _ in 0..20 {
        let (_, profile) = client.profile(|| launch(out_h.clone()), "matmul_fp4_ldm2").unwrap();

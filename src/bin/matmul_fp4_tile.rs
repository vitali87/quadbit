//! Block-tile-shape experiment for the fed FP4 matmul. The fed kernel is
//! OMMA-latency bound, but its staging cost (LDG+STS) is amortized over only the
//! per-block output tile, and a wider/squarer block tile raises the staging
//! arithmetic intensity BM*BN / ((BM+BN)*BK): the 4-warp 64x128 tile is 0.33, the
//! 8-warp square 128x128 tile is 0.50, so the square block issues ~33% fewer
//! staging loads per output. Both run 8 warps/SM at 255 regs/thread (the 4-warp
//! config as 2 blocks/SM, the 8-warp as 1 block/SM), so occupancy and the
//! register ceiling are held fixed; only the staging-per-output ratio changes.
//! This times the two interleaved (same clock state) to see whether staging is
//! any part of the wall or fully hidden under OMMA latency.
//!
//! The per-warp work is identical in both (WM=2 x WN=8 = 16 accumulator chains);
//! only the warp grid (WARPS_M x WARPS_N) and the derived block dims differ, so
//! the kernel is comptime-generic over the warp grid.
...
const SF: usize = 2;
const WM: usize = 2; // MMA fragments per warp in m (fixed: 2x8 = 16 chains)
...
fn fed<A: Scalar, B: Scalar, CD: Numeric, S: Scalar, NA: Size, NB: Size, NC: Size>(
    ...
    #[comptime] warps_m: usize,
    #[comptime] warps_n: usize,
) {
    ...
    let bm = comptime!(warps_m * WM * MMA_M);
    let bn = comptime!(warps_n * WN * MMA_N);
    let n_threads = comptime!(warps_m * warps_n * 32);
    ...
    let mut a_tile = SharedMemory::<Vector<A, NA>>::new(comptime!(bm * VPR));
    let mut b_tile = SharedMemory::<Vector<B, NB>>::new(comptime!(bn * VPR));
    let bps = comptime!(BK / (MMA_K / SF));
    let mut sa_tile = SharedMemory::<S>::new(comptime!(bm * bps));
    let mut sb_tile = SharedMemory::<S>::new(comptime!(bn * bps));
...
// (warps_m, warps_n): 4-warp 64x128 (current fed) vs 8-warp 128x128 square.
const VARIANTS: [(usize, usize); 2] = [(2, 2), (4, 2)];
...
fn run<R: Runtime>(device: &R::Device) {
    ...
    let launch = |wmn: (usize, usize), m: usize, n: usize, k: usize, lhs_h: &cubecl::server::Handle, rhs_h: &cubecl::server::Handle, lsb_h: &cubecl::server::Handle, rsb_h: &cubecl::server::Handle, c_h: &cubecl::server::Handle, out_h: cubecl::server::Handle| {
        let (warps_m, warps_n) = wmn;
        let bm = warps_m * WM * MMA_M;
        let bn = warps_n * WN * MMA_N;
        let n_threads = (warps_m * warps_n * 32) as u32;
        let scale_cols = k / (MMA_K / SF);
        unsafe {
    ...
    for wmn in VARIANTS {
        ...
        launch(wmn, vm, vn, vk, &lhs_h, &rhs_h, &lsb_h, &rsb_h, &c_h, out_h.clone());
        ...
        println!(
            "verify warps={wmn:?}: {} ({wrong} wrong, max_rel={max_rel:.2e})",
    ...
    for wmn in VARIANTS {
        for _ in 0..5 {
            launch(wmn, m, n, k, &lhs_h, &rhs_h, &lsb_h, &rsb_h, &c_h, out_h.clone());
        }
    }
    ...
    for _ in 0..30 {
        for (idx, wmn) in VARIANTS.iter().enumerate() {
    ...
    println!("warps {:?} (64x128): {:.3} ms  {g0:.1} GFLOP/s", VARIANTS[0], best[0].as_secs_f64() * 1e3);
    println!("warps {:?} (128x128): {:.3} ms  {g1:.1} GFLOP/s", VARIANTS[1], best[1].as_secs_f64() * 1e3);
    println!("speedup (square/base): {:.4}x", g1 / g0);
}

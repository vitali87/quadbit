//! Milestone 4: warp-specialized + multi-stage pipelined FP4 matmul. A dedicated
//! producer warp issues ONLY TMA loads into a 2-stage shared ring buffer; four
//! consumer warps drain it with FP4 ldmatrix + mma.sync (the proven 2x8 tile from
//! ldm2). The producer/consumer mbarrier handshake (full/empty + phase wait_parity)
//! is validated in `src/bin/pc_probe.rs`; the consumer compute is ldm2's, the load
//! recipe is from `ldm_probe`. This is the structural technique behind CUTLASS's 3x
//! (KernelTmaWarpSpecializedPingpong): consumers spend their whole budget on MMA
//! while TMA latency hides under the ring. Scales are read straight from global in
//! the consumers. Verified against the same f32 reference as fed.
...
const CONSUMER_WARPS: usize = WARPS_M * WARPS_N; // 4
const WM: usize = 2;
const WN: usize = 8;
const BM: usize = WARPS_M * WM * MMA_M; // 64
const BN: usize = WARPS_N * WN * MMA_N; // 128
const BK: usize = 128;
const BKH: usize = BK / 2; // e2m1x2 bytes per k-step row
const KSUB: usize = BK / MMA_K;
const STAGES: usize = 2;
const N_THREADS: u32 = ((CONSUMER_WARPS + 1) * 32) as u32; // 4 consumers + 1 producer = 160
const CONSUMER_THREADS: u32 = (CONSUMER_WARPS * 32) as u32; // 128
const A_STAGE: usize = BM * BKH; // 4096 e2m1x2
const B_STAGE: usize = BN * BKH; // 8192

#[cube(launch_unchecked)]
fn matmul_fp4_ws2<A: Scalar, B: Scalar, CD: Numeric, S: Scalar, NA: Size, NB: Size, NC: Size>(
    a: &TensorMap<A, Tiled>,
    b: &TensorMap<B, Tiled>,
    ...
    let ab_bytes = select(lane_id == 0, comptime!((A_STAGE + B_STAGE) as u32), 0u32);
    let full0 = Barrier::shared(1, UNIT_POS_X == 0);
    let full1 = Barrier::shared(1, UNIT_POS_X == 0);
    let empty0 = Barrier::shared(CONSUMER_THREADS, UNIT_POS_X == 0);
    let empty1 = Barrier::shared(CONSUMER_THREADS, UNIT_POS_X == 0);
    sync_async_proxy_shared();
    let mut a_tile = SharedMemory::<A>::new_aligned(STAGES * A_STAGE, 128usize);
    ...
    if warp >= CONSUMER_WARPS {
        ...
        if lane_id == 0 {
            ...
            full0.tma_load_2d(a, &mut a0, block_row as i32, 0);
            let mut b0 = b_tile.slice_mut(0, B_STAGE);
            full0.tma_load_2d(b, &mut b0, block_col as i32, 0);
            full0.arrive_and_expect_tx(1, ab_bytes);
            let mut a1 = a_tile.slice_mut(A_STAGE, 2 * A_STAGE);
            full1.tma_load_2d(a, &mut a1, block_row as i32, BKH as i32);
            let mut b1 = b_tile.slice_mut(B_STAGE, 2 * B_STAGE);
            full1.tma_load_2d(b, &mut b1, block_col as i32, BKH as i32);
            full1.arrive_and_expect_tx(1, ab_bytes);
            for step in 2..k_steps {
                let col = (step * BKH) as i32;
                let parity = ((step / 2 - 1) % 2) as u32;
                if step % 2 == 0 {
                    empty0.wait_parity(parity);
                    let mut sa = a_tile.slice_mut(0, A_STAGE);
                    full0.tma_load_2d(a, &mut sa, block_row as i32, col);
                    let mut sb = b_tile.slice_mut(0, B_STAGE);
                    full0.tma_load_2d(b, &mut sb, block_col as i32, col);
                    full0.arrive_and_expect_tx(1, ab_bytes);
                } else {
                    empty1.wait_parity(parity);
                    let mut sa = a_tile.slice_mut(A_STAGE, 2 * A_STAGE);
                    full1.tma_load_2d(a, &mut sa, block_row as i32, col);
                    let mut sb = b_tile.slice_mut(B_STAGE, 2 * B_STAGE);
                    full1.tma_load_2d(b, &mut sb, block_col as i32, col);
                    full1.arrive_and_expect_tx(1, ab_bytes);
                }
            }
        }
        ...
        let vc_a = def.vectors_per_lane(MatrixIdent::A);
        let vc_b = def.vectors_per_lane(MatrixIdent::B);
        ...
        let la_arow = ((lane_id / 8) % 2) as usize * 8 + (lane_id % 8) as usize;
        let la_cblk = ((lane_id / 8) / 2) as usize;
        let lb_nrow = (lane_id % 8) as usize;
        let lb_kblk = ((lane_id / 8) % 2) as usize;
        ...
        for i in 0..vector_count_c {
            ...
            let row0 = (block_row + a_row0 + row as usize) * full_n;
            let row1 = (block_row + a_row1 + row as usize) * full_n;
            let cc = col as usize;
        ...
        let mut a0 = Array::<Vector<A, NA>>::new(vc_a);
        let mut a1 = Array::<Vector<A, NA>>::new(vc_a);
        let mut b0 = Array::<Vector<B, NB>>::new(vc_b);
        let mut b1 = Array::<Vector<B, NB>>::new(vc_b);
        let mut b2 = Array::<Vector<B, NB>>::new(vc_b);
        let mut b3 = Array::<Vector<B, NB>>::new(vc_b);
        let mut b4 = Array::<Vector<B, NB>>::new(vc_b);
        let mut b5 = Array::<Vector<B, NB>>::new(vc_b);
        let mut b6 = Array::<Vector<B, NB>>::new(vc_b);
        let mut b7 = Array::<Vector<B, NB>>::new(vc_b);
        ...
        for step in 0..k_steps {
            ...
            let astage = (step % 2) * A_STAGE;
            let bstage = (step % 2) * B_STAGE;
            if step % 2 == 0 {
            ...
            #[unroll]
            for ks in 0..KSUB {
                let kb = ks * (MMA_K / 2); // byte col within the staged row
                let sk = ks * SF;
                let a0s = astage + (a_row0 + la_arow) * BKH + kb + la_cblk * 16;
                let asl0 = a_tile.slice(a0s, a0s + 16);
                def.load_matrix_inplace::<A, NA>(&asl0, &mut a0, MatrixIdent::A, vc_a, false);
                let a1s = astage + (a_row1 + la_arow) * BKH + kb + la_cblk * 16;
                let asl1 = a_tile.slice(a1s, a1s + 16);
                def.load_matrix_inplace::<A, NA>(&asl1, &mut a1, MatrixIdent::A, vc_a, false);

                let b0s = bstage + (b_col0 + lb_nrow) * BKH + kb + lb_kblk * 16;
                let bs0 = b_tile.slice(b0s, b0s + 16);
                def.load_matrix_inplace::<B, NB>(&bs0, &mut b0, MatrixIdent::B, vc_b, false);
                let b1s = bstage + (b_col1 + lb_nrow) * BKH + kb + lb_kblk * 16;
                let bs1 = b_tile.slice(b1s, b1s + 16);
                def.load_matrix_inplace::<B, NB>(&bs1, &mut b1, MatrixIdent::B, vc_b, false);
                let b2s = bstage + (b_col2 + lb_nrow) * BKH + kb + lb_kblk * 16;
                let bs2 = b_tile.slice(b2s, b2s + 16);
                def.load_matrix_inplace::<B, NB>(&bs2, &mut b2, MatrixIdent::B, vc_b, false);
                let b3s = bstage + (b_col3 + lb_nrow) * BKH + kb + lb_kblk * 16;
                let bs3 = b_tile.slice(b3s, b3s + 16);
                def.load_matrix_inplace::<B, NB>(&bs3, &mut b3, MatrixIdent::B, vc_b, false);
                let b4s = bstage + (b_col4 + lb_nrow) * BKH + kb + lb_kblk * 16;
                let bs4 = b_tile.slice(b4s, b4s + 16);
                def.load_matrix_inplace::<B, NB>(&bs4, &mut b4, MatrixIdent::B, vc_b, false);
                let b5s = bstage + (b_col5 + lb_nrow) * BKH + kb + lb_kblk * 16;
                let bs5 = b_tile.slice(b5s, b5s + 16);
                def.load_matrix_inplace::<B, NB>(&bs5, &mut b5, MatrixIdent::B, vc_b, false);
                let b6s = bstage + (b_col6 + lb_nrow) * BKH + kb + lb_kblk * 16;
                let bs6 = b_tile.slice(b6s, b6s + 16);
                def.load_matrix_inplace::<B, NB>(&bs6, &mut b6, MatrixIdent::B, vc_b, false);
                let b7s = bstage + (b_col7 + lb_nrow) * BKH + kb + lb_kblk * 16;
                let bs7 = b_tile.slice(b7s, b7s + 16);
                def.load_matrix_inplace::<B, NB>(&bs7, &mut b7, MatrixIdent::B, vc_b, false);

                let kblk = step * bps + sk;
                #[unroll]
                for i in 0..scales_count {
                    sa0[i] = scales_a[(block_row + a_row0 + scales_idx_a) * scale_blocks_per_row + kblk + i];
                    sa1[i] = scales_a[(block_row + a_row1 + scales_idx_a) * scale_blocks_per_row + kblk + i];
                    sb0[i] = scales_b[(block_col + b_col0 + scales_idx_b) * scale_blocks_per_row + kblk + i];
                    sb1[i] = scales_b[(block_col + b_col1 + scales_idx_b) * scale_blocks_per_row + kblk + i];
                    sb2[i] = scales_b[(block_col + b_col2 + scales_idx_b) * scale_blocks_per_row + kblk + i];
                    sb3[i] = scales_b[(block_col + b_col3 + scales_idx_b) * scale_blocks_per_row + kblk + i];
                    sb4[i] = scales_b[(block_col + b_col4 + scales_idx_b) * scale_blocks_per_row + kblk + i];
                    sb5[i] = scales_b[(block_col + b_col5 + scales_idx_b) * scale_blocks_per_row + kblk + i];
                    sb6[i] = scales_b[(block_col + b_col6 + scales_idx_b) * scale_blocks_per_row + kblk + i];
                    sb7[i] = scales_b[(block_col + b_col7 + scales_idx_b) * scale_blocks_per_row + kblk + i];
                }
...
fn verify<R: Runtime>(device: &R::Device) {
    ...
    let supported = client.features().matmul.scaled_mma.contains(&ScaledMmaConfig {
        a_type: ab_elem, b_type: ab_elem, cd_type: f32::cube_type(), scales_type: S::cube_type(),
        m: MMA_M as u32, n: MMA_N as u32, k: MMA_K as u32, scales_factor: SF as u32,
    });
    if !supported { println!("verify: unsupported; skipping"); return; }
    ...
    let c_h = client.create_from_slice(f32::as_bytes(&vec![0.0f32; m * n]));
    let out_h = client.empty(m * n * core::mem::size_of::<f32>());
    unsafe {
        matmul_fp4_ws2::launch_unchecked::<AB, AB, f32, S, R>(
            &client, CubeCount::Static((n / BN) as u32, (m / BM) as u32, 1), CubeDim::new_1d(N_THREADS),
            ab_vs, ab_vs, 2,
            a_map::<R>(lhs_h, m, k, BM), a_map::<R>(rhs_h, n, k, BN),
...
fn run_size<R: Runtime>(device: &R::Device, sz: usize) {
    ...
    let launch = |out_buf: cubecl::server::Handle| unsafe {
        matmul_fp4_ws2::launch_unchecked::<AB, AB, f32, S, R>(
            &client, CubeCount::Static((n / BN) as u32, (m / BM) as u32, 1), CubeDim::new_1d(N_THREADS),
            ab_vs, ab_vs, 2,
            a_map::<R>(lhs_h.clone(), m, k, BM), a_map::<R>(rhs_h.clone(), n, k, BN),
    ...
    for _ in 0..5 { launch(out_h.clone()); }
    let _ = future::block_on(client.sync());
    let mut best = Duration::MAX;
    for _ in 0..20 {
        let (_, profile) = client.profile(|| launch(out_h.clone()), "matmul_fp4_ws2").unwrap();
    ...
    let mut wrong = 0;
    for v in result.iter() { if (*v - expected).abs() > 0.03 * expected { wrong += 1; } }
...
fn main() {
    #[cfg(feature = "cuda")]
    {
        ...
        for sz in [2048usize, 4096, 8192] { run_size::<cubecl::cuda::CudaRuntime>(&Default::default(), sz); }
    }

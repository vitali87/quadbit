//! Same-invocation A/B benchmark harness for the fed FP4 matmul. Absolute
//! GFLOP/s on Modal drifts with GPU clock boost between invocations, so
//! cross-run deltas are unreliable; this times two kernel variants INTERLEAVED
//! in one process (best-of-N each) so they share clock state, and verifies both
//! against an f32 reference on random representable FP4 + random per-block
//! scales. Reuse it to vet any candidate change before committing.
//!
//! The two variants are one kernel specialized on `#[comptime] hoist_a`:
//!   - false: load A and B fragments via `position_of_nth` in the k-loop
//!   - true:  precompute all fragment shared offsets once and just add the
//!            per-sub-step stride in the loop (cut per-OMMA address arithmetic)
//!
//! Result: hoisting is NEUTRAL (~1.00x). The kernel is OMMA-latency bound and
//! the integer address math overlaps that latency for free, so removing it does
//! not help (unlike scale-staging, which removed memory-pipe LDGs). This also
//! debunked an earlier cross-run "regression" that was just clock noise.
...
fn fed<A: Scalar, B: Scalar, CD: Numeric, S: Scalar, NA: Size, NB: Size, NC: Size>(
    ...
    // precomputed A-fragment shared offsets (used only when hoist_a; otherwise
    // pure + unread, so dead-code-eliminated)
    let ksub_stride = comptime!(MMA_K / FV);
    let mut a_off0 = Array::<u32>::new(vector_count_a);
    let mut a_off1 = Array::<u32>::new(vector_count_a);
    #[unroll]
    for i in 0..vector_count_a {
        ...
        a_off0[i] = (((a_row0 + row as usize) * BK + col as usize) / FV) as u32;
        a_off1[i] = (((a_row1 + row as usize) * BK + col as usize) / FV) as u32;
    }
    let mut b_off0 = Array::<u32>::new(vector_count_b);
    let mut b_off1 = Array::<u32>::new(vector_count_b);
    let mut b_off2 = Array::<u32>::new(vector_count_b);
    let mut b_off3 = Array::<u32>::new(vector_count_b);
    let mut b_off4 = Array::<u32>::new(vector_count_b);
    let mut b_off5 = Array::<u32>::new(vector_count_b);
    let mut b_off6 = Array::<u32>::new(vector_count_b);
    let mut b_off7 = Array::<u32>::new(vector_count_b);
    #[unroll]
    for i in 0..vector_count_b {
        ...
        let rr = row as usize;
        b_off0[i] = (((b_col0 + col as usize) * BK + rr) / FV) as u32;
        b_off1[i] = (((b_col1 + col as usize) * BK + rr) / FV) as u32;
        b_off2[i] = (((b_col2 + col as usize) * BK + rr) / FV) as u32;
        b_off3[i] = (((b_col3 + col as usize) * BK + rr) / FV) as u32;
        b_off4[i] = (((b_col4 + col as usize) * BK + rr) / FV) as u32;
        b_off5[i] = (((b_col5 + col as usize) * BK + rr) / FV) as u32;
        b_off6[i] = (((b_col6 + col as usize) * BK + rr) / FV) as u32;
        b_off7[i] = (((b_col7 + col as usize) * BK + rr) / FV) as u32;
    }
    ...
    for step in 0..k_steps {
        ...
        for i in 0..(BM * VPR) / n_threads {
            ...
            a_tile[s] = a[(block_row + s / VPR) * global_vpr + k_vec + s % VPR];
        }
        #[unroll]
        for i in 0..(BN * VPR) / n_threads {
            let s = tid + i * n_threads;
            b_tile[s] = b[(block_col + s / VPR) * global_vpr + k_vec + s % VPR];
        }
        #[unroll]
        ...
        for ks in 0..KSUB {
            ...
            for i in 0..vector_count_a {
                if hoist_a {
                    a0[i] = a_tile[a_off0[i] as usize + ks * ksub_stride];
                    a1[i] = a_tile[a_off1[i] as usize + ks * ksub_stride];
                } else {
                    let n_elem = i * vector_size_a * a_pack;
                    let (row, col) = def.position_of_nth(lane_id, n_elem as u32, MatrixIdent::A);
            ...
            for i in 0..vector_count_b {
                if hoist_a {
                    let ko = ks * ksub_stride;
                    b0[i] = b_tile[b_off0[i] as usize + ko];
                    b1[i] = b_tile[b_off1[i] as usize + ko];
                    b2[i] = b_tile[b_off2[i] as usize + ko];
                    b3[i] = b_tile[b_off3[i] as usize + ko];
                    b4[i] = b_tile[b_off4[i] as usize + ko];
                    b5[i] = b_tile[b_off5[i] as usize + ko];
                    b6[i] = b_tile[b_off6[i] as usize + ko];
                    b7[i] = b_tile[b_off7[i] as usize + ko];
                } else {
                    let n_elem = i * vector_size_b * b_pack;
                    let (row, col) = def.position_of_nth(lane_id, n_elem as u32, MatrixIdent::B);
...
fn run<R: Runtime>(device: &R::Device) {
    ...
    // build inputs at a given size; returns device handles + host refs for checking
    ...
    let launch = |hoist: bool, m: usize, n: usize, k: usize, lhs_h: &cubecl::server::Handle, rhs_h: &cubecl::server::Handle, lsb_h: &cubecl::server::Handle, rsb_h: &cubecl::server::Handle, c_h: &cubecl::server::Handle, out_h: cubecl::server::Handle| {
        let scale_cols = k / (MMA_K / SF);
        unsafe {
    ...
    // correctness for both variants at 512^3
    ...
    for hoist in [false, true] {
        ...
        launch(hoist, vm, vn, vk, &lhs_h, &rhs_h, &lsb_h, &rsb_h, &c_h, out_h.clone());
        ...
        println!(
            "verify hoist_a={hoist}: {} ({wrong} wrong, max_rel={max_rel:.2e})",
            if wrong == 0 { "PASS" } else { "FAIL" }
        );
    }

    // interleaved timing at 4096^3 (same invocation -> same clock state)
    ...
    for hoist in [false, true] {
        for _ in 0..5 {
            launch(hoist, m, n, k, &lhs_h, &rhs_h, &lsb_h, &rsb_h, &c_h, out_h.clone());
        }
    }
    ...
    for _ in 0..30 {
        for (idx, hoist) in [false, true].iter().enumerate() {
    ...
    println!("hoist_a=false: {:.3} ms  {g0:.1} GFLOP/s", best[0].as_secs_f64() * 1e3);
    println!("hoist_a=true : {:.3} ms  {g1:.1} GFLOP/s", best[1].as_secs_f64() * 1e3);
    println!("speedup (hoist/base): {:.4}x", g1 / g0);
}

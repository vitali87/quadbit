//! Milestone 1 toward closing the 3x gap to CUTLASS: replace the fed kernel's
//! manual cooperative global staging with TMA (`TensorMap` + `Barrier::tma_load`).
//! The CUTLASS Sm120 nvfp4 reference reaches ~3x our throughput; its SASS shows
//! the staging is 4 `UTMALDG` (TMA) instructions, which erase the 62 LDG and the
//! integer/address arithmetic that is 58% of our fed kernel's instruction stream.
//!
//! This kernel keeps fed's proven 2x8 warp tile and MMA inner loop byte for byte;
//! the ONLY change is the load path: A and B are now `&TensorMap<.,Tiled>` staged
//! with one `tma_load_2d` each per k-step into 128-byte-aligned shared, gated by an
//! mbarrier transaction (`arrive_and_expect_tx` / `wait`). Scales stay on the
//! manual path for now (TMA-staged and the multi-stage pipeline come in milestones
//! 2+). Verified against the same f32 reference as fed.
...
fn matmul_fp4_pipe<A: Scalar, B: Scalar, CD: Numeric, S: Scalar, NA: Size, NB: Size, NC: Size>(
    a: &TensorMap<A, Tiled>,
    b: &TensorMap<B, Tiled>,
    ...
    // TMA needs 128-byte-aligned shared destinations.
    let mut a_tile = SharedMemory::<Vector<A, NA>>::new_aligned(BM * VPR, 128usize);
    let mut b_tile = SharedMemory::<Vector<B, NB>>::new_aligned(BN * VPR, 128usize);
    ...
    for step in 0..k_steps {
        ...
        for ks in 0..KSUB {
            ...
            for i in 0..vector_count_a {
                ...
                a0[i] = a_tile[((a_row0 + row as usize) * BK + k_off + col as usize) / (div * vector_size_a)];
                a1[i] = a_tile[((a_row1 + row as usize) * BK + k_off + col as usize) / (div * vector_size_a)];
            }
            #[unroll]
            ...
            for i in 0..vector_count_b {
                ...
                let div = b_pack;
                b0[i] = b_tile[((b_col0 + col as usize) * BK + k_off + row as usize) / (div * vector_size_b)];
                b1[i] = b_tile[((b_col1 + col as usize) * BK + k_off + row as usize) / (div * vector_size_b)];
                b2[i] = b_tile[((b_col2 + col as usize) * BK + k_off + row as usize) / (div * vector_size_b)];
                b3[i] = b_tile[((b_col3 + col as usize) * BK + k_off + row as usize) / (div * vector_size_b)];
                b4[i] = b_tile[((b_col4 + col as usize) * BK + k_off + row as usize) / (div * vector_size_b)];
                b5[i] = b_tile[((b_col5 + col as usize) * BK + k_off + row as usize) / (div * vector_size_b)];
                b6[i] = b_tile[((b_col6 + col as usize) * BK + k_off + row as usize) / (div * vector_size_b)];
                b7[i] = b_tile[((b_col7 + col as usize) * BK + k_off + row as usize) / (div * vector_size_b)];
            }
            #[unroll]
            for i in 0..scales_count {
...
fn verify<R: Runtime>(device: &R::Device) {
    ...
    unsafe {
        matmul_fp4_pipe::launch_unchecked::<AB, AB, f32, S, R>(
            &client,
...
fn run_size<R: Runtime>(device: &R::Device, sz: usize) {
    ...
    for _ in 0..20 {
        let (_, profile) = client.profile(|| launch(out_h.clone()), "matmul_fp4_pipe").unwrap();

...
pub fn ldmatrix_call<D: Dialect>(
    ...
    let regs = if elem.size() == 1 {
        ...
        comma_separated((0..reg_count).map(|i| format!("reinterpret_cast<uint32&>({output}[{i}])")))
    };
...
pub fn ldmatrix_template<D: Dialect>(elem: Elem<D>, factor: u32, transpose: bool) -> String {
    ...
    let placeholders_regs = comma_separated((0..reg_count).map(|_| placeholder(&mut idx)));
    let placeholders_regs = format!("{{{placeholders_regs}}}");

    let placeholder_addr = format!("[{}]", placeholder(&mut idx));

    let params_regs = comma_separated((0..reg_count).map(|i| format!(r#""=r"(reg_{i})"#)));
...
__ldmatrix_m{width}n8_{elem}_{factor}x{is_transposed}({args}) {{
  ...
  asm volatile("ldmatrix.sync.aligned.{shape}.{num}{transposed_arg}.shared::cta.{ty}"

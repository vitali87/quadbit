...
downstream regression. Two honest limits remain: this GLM downstream
evidence is a small 4-task smoke suite on the D2 policy only (full
benchmarks, and downstream numbers for the down-only/gate-up rows, are
still unmeasured on GLM, so the most complete capability accounting of
record remains DeepSeek's); and all GLM rows run eager, because the
plugin's expert-parallel local-expert loop uses a host-syncing

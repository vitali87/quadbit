...
## B200 head-to-head: the SM120 decode numbers are an interconnect artifact, 3.27x (2026-07-24, branch `b200-glm-headtohead` PR #38, `docs/b200/verdict.md`)
...
- **Same checkpoint, same harness, same passage (`ntok=96` both rows), same captured graph mode, same commit; only the silicon differs.** 8x RTX PRO 6000 (PCIe) **34.305 tok/s** (reproduces the frozen 33.810 within 1.5%, so not toolchain drift) vs **4x B200 (NVLink, vanilla vLLM, `QB_DENSE=off`) 112.079 tok/s = 3.27x**, PPL a wash (3.9168 vs 3.7352 on 96 tokens), both captured PASS.

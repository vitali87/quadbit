...
- **C9 REVERSES on NVLink.** The MTP KILL (-17.8% on SM120, and spec=1 deadlocked 66 min at 0 tok/s) was driven by each draft step paying the per-layer 374 us all-reduce. At ~80 us it pays for itself: **B200 MTP spec=1 = 181.278 tok/s vs 112.079 no-spec = +61.7%, lossless (PPL identical to 4dp, 3.7352 = 3.7352)**, captured PASS, clean completion. **Cumulative 34.305 -> 181.278 = 5.28x** from an interconnect change plus one config flag, no kernel/quant/policy change. Gap to Aster's 281 goes **8.19x -> 2.51x -> 1.55x**. Caveat: C9's deadlock was DeepSeek on SM120 vs GLM on B200 here, so two variables changed; the spec=2 row (C9's exact spec count) is the closer comparison. Do NOT multiply Baseten's 2x PD-disaggregation onto this: PD disagg buys aggregate throughput/TTFT under concurrent load, while this is single-stream at batch 1-2.

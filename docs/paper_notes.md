...
- **Verdict: per-expert gate_up STE-QAT layerwise repair does NOT recover.** Same lesson as Gap C full-stack QAT (`docs/qat/design.md`) and A3: local per-expert reconstruction rel does not predict downstream; the 2:4-FP4 gate_up capability floor is real, not a repair artifact. The result VALIDATES the deployed strategy - you AVOID the gate_up tax by anchoring gate_up dense (down49), you do not repair it. down49 stays the cleanest capability-preserving policy. True global MoE QAT (loss on the routed aggregate) is untested and remains the harder, infra-gated lever.

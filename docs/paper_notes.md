...
- **Down-only does not scale to full coverage** (early-layer down-sparse reintroduces compounding, same as finding (1)): c_down74 (74% layer) .7069, c_down100 (100% layer) .6502 — both still UNIFORMLY beat full-sparse at the same coverage (a2_74 .6246 -> +8pt; a2_100 .5092 -> +14pt) but miss .718. The .718 boundary for down-only sits between 49% (.7354, +1.7pt margin) and 74% (.7069); pinning the max coverage that clears it (c_down60/c_down65 running).

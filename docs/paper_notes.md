...
- Compute split (event-time, summed over ranks, whole run): expert-kernel ~ dense/attn (~52k vs ~49k ms @2-GPU; ~119k vs ~110k ms @4-GPU) => the sparse expert path is ~half of instrumented GPU time, not a bottleneck vs attention; the ~475 ms/tok wall is eager + per-layer Python routing, no CUDA graph (CUDAGraphMode.NONE, enforce_eager). Graph capture of the sparse path was the M3 speed unlock.
- **M3 UPDATE (LANDED, commits `603f9b4` M3-A / `f8d3c45` M3-B / `362d4b7` graph-wire / `d896795` vLLM full-capture; C1 native delegation):** the sparse decode path IS now CUDA-graph-capturable — `route_fixed_cap` (scatter_add counts, where/scatter routing, device-scalar drop) replaces `build_routing`'s `.item()` host loop, and the stream-variant kernels skip `cudaDeviceSynchronize` (`sparse_fp4_lib.cu:1039`). Captures are bit-faithful and memory-bounded (pool ~2 GiB). **Decode speed splits by policy shape:** route-slot **D2** (whole-expert dense slots) is delegated to FlashInfer `group_gemm_nvfp4_nt_groupwise` → **captured 5.82 tok/s** (11.3x over the dequant loop, C1 [c1/verdict.md](c1/verdict.md)); **down49** (per-projection anchoring, sparse `down_proj` + dense `gate_up` in one layer) canNOT use whole-layer native delegation, so its 43 anchored layers keep the `range(E)` dequant loop → **~0.66 tok/s** ([p4/m4d_down49_verdict.md](p4/m4d_down49_verdict.md)). The named fix for down49 is a **per-projection dense NVFP4 grouped-GEMM kernel (DIY)**. NOTE: even D2's 5.82 tok/s is ~10x below the dense-native C4 SOTA (58.126) — at batch-1 decode the sparse policy is floor-bound (MoE ~5.5% of decode) and our grouped-GEMM anchors are heavier than FlashInfer-CUTLASS fused MoE, so the sparse serving win is a compute-bound/throughput + memory story, not a decode-latency win.
...
- Conclusion: sparse-FP4 experts serve end-to-end and are memory/throughput-measurable; does NOT scale 2->4 GPU for speed (comm-bound on PCIe, no NVLink), only for KV capacity; quality tax explained. Blocker (1) CUDA-graph-capturable sparse path is RESOLVED (see M3 UPDATE above; D2 native-captured 5.82 tok/s, down49 needs a DIY per-projection grouped-GEMM). Remaining: (2) chunked-prefill engine crash at long context, (3) training-free quality tax (QAT/selective-layer), and the open strategic question = the sparse serving win is compute-bound (prefill/high-concurrency/large-M), NOT batch-1 decode, so the deployment win must be demonstrated in that regime.
...
## WS-E downstream breadth: 8-task battery, both models, all deployed policies (2026-07-21, branch `ws-e-downstream-breadth`, `docs/wse/verdict.md`, `/cache/qb_downstream_wse_*.csv`)
Hardened the quality evidence against two gaps: the frozen battery was only 4 tasks (PIQA symptom-dropped), and GLM's down49 was PPL-only. Widened to 8 tasks (added ARC-Easy, OpenBookQA, BoolQ, and PIQA restored at root cause via the HF parquet-convert branch `revision=refs/convert/parquet` - no gated loading script on the serve image), 400 items/task, on both models x 3 deployed policies (dense / down49 / D2 route-slot).
- **8-task AVG (normalized primary):** DeepSeek dense **.7548**, down49 .7491 (-0.57pt), D2 .7551 (**+0.03pt**); GLM dense **.7841**, down49 .7826 (**-0.15pt**), D2 .7762 (-0.79pt).
- **No task collapses anywhere** (largest single-task move DeepSeek down49 winogrande -3.0pt, GLM D2 arc_c -2.0pt, both inside the per-400 band). Restored PIQA is neutral-to-positive under sparsity (GLM down49 .878 ABOVE dense .868), confirming it never masked a regression - the earlier drop was a loader artifact, not a capability signal.
- **GLM's PPL-only caveat on down49 closes:** downstream -0.15pt (near-flat) confirms the down-tolerant rule on GLM with accuracy, not just the +0.209 PPL. Only the gateup49 CONTROL stays PPL-only.
- **The best sparse policy differs by model:** DeepSeek D2 (+0.03) edges down49; GLM down49 (-0.15) beats D2. Both models keep both policies near dense. The 8-task battery is generally KINDER than the 4-task (added tasks are ones sparse holds on); DeepSeek D2 reads +0.03pt here vs the frozen -0.79pt 4-task row = run-to-run MC variance + battery composition, NOT a headline upgrade. Frozen `deepseek_final.csv` / limit=200 GLM rows remain the paper reference; WS-E corroborates "no collapse on a wider battery."

## gate_up QAT recovery: KILL (2026-07-22, branch `qat-gateup-recovery`, `docs/qat/gateup_moe_verdict.md`)
Settles the last open QAT question from WS-E ("only the gateup49 CONTROL stays PPL-only"; can QAT repair the gate_up tax?). Per-expert gate_up-only STE-QAT layerwise repair on the sparse-trajectory serve-consistent teacher (reio2, layers 22-42, 300 STE steps, lr=1e-4, down held dense-exact) on DeepSeek-V4-Flash-NVFP4, evaluated on the same WS-E 8-task limit=400 battery in-process. NOTE the objective is per-expert (each expert fit to its own dense output, `moe_recon.py:164`), NOT the router-weighted top-k combine; it improves on A3 via serve-consistent inputs + a stable lr, not via global consistency.
- **8-task AVG (normalized primary):** one-shot gateup49 (no QAT, new control) **.7363**; **QAT gateup49 .7332 = -0.31pt** (within MC noise, SE ~0.8pt). Per-expert STE fit was tight (gate_up-vs-dense rel 0.02-0.07 all 21 layers) yet downstream did not move; only arc_c is outside noise and QAT is WORSE there.
- **Verdict: per-expert gate_up STE-QAT layerwise repair does NOT recover.** Same lesson as Gap C full-stack QAT (`docs/qat/design.md`) and A3: local per-expert reconstruction rel does not predict downstream; the 2:4-FP4 gate_up capability floor is real, not a repair artifact. The result VALIDATES the deployed strategy - you AVOID the gate_up tax by anchoring gate_up dense (down49), you do not repair it. down49 stays the cleanest 2-GPU capability-preserving policy (DeepSeek D2 edges it on 8-task quality, +0.03 vs -0.57pt, but needs 4 GPUs; preferred sparse policy varies by model, see line 709). The router-weighted global MoE QAT objective (loss on the routed aggregate) was the untested lever here; it is now settled KILL below.
- **Method caution:** the QAT .7332 first looked like a WIN vs the .7056 gateup49 baseline, but .7056/.718-bar are 4-task limit=200 (frozen ref) while this run is 8-task limit=400 (kinder) - cross-battery compare is invalid. The apples-to-apples control (one-shot gateup49 8-task, .7363) had to be run fresh; DeepSeek gateup49 now has 8-task downstream (GLM's gateup49 control stays PPL-only).

## global MoE QAT recovery: KILL (2026-07-22, PR #34 merged `98a5a41`, `docs/qat/global_moe_verdict.md`)
The harder lever the per-expert KILL flagged: fit the **router-weighted top-k combine** to the **dense routed aggregate** (what the next layer sees), using the routing weights the per-expert objective discarded. `serve_dsv4.py::recon --recon-mode global --rounds 3`, `QB_SPARSE_PROJ=gateup`, reio2, layers 22-42, 200 steps/expert/round x 3 rounds, lr=1e-4, DeepSeek-V4-Flash-NVFP4, 2x RTX PRO 6000 TP=2. Trainer `moe_recon.train_layer_global`: Gauss-Seidel one-expert-resident coordinate descent (joint 256-expert backward OOMs; Jacobi diverges, L23 0.58->2.43), in-place residual update, best-round keep, dense teacher rebuilt from dumped x/tid/tw.
- **8-task AVG (normalized primary): global QAT gateup49 .7259 = -1.04pt vs one-shot .7363, -0.73pt vs per-expert QAT .7332** (both at/beyond MC noise SE ~0.8pt). The global objective gave the TIGHTEST reconstruction of all three variants (routed-aggregate rel driven monotonically to 0.004-0.017, ~4-10x tighter than per-expert's 0.02-0.07) yet the WORST downstream. Per-task: arc_c .6125, arc_e .8625, hellaswag .6750, piqa .8300, obqa .4350, boolq .8350, winogrande .7650, mmlu-5 .7920; PPL 3.777.
- **Verdict: true global MoE QAT does NOT recover — worse than both weaker variants despite the tightest fit.** Closes the last open QAT lever. Strongest confirmation yet of the recurring lesson (Gap C, A3, per-expert gate_up): local reconstruction rel does not predict downstream, and driving it TIGHTER makes the mismatch worse. No layerwise-repair objective (own-output, routed-aggregate, dense- or sparse-trajectory) clears the 2:4-FP4 gate_up floor. You AVOID the tax by anchoring gate_up dense (down49), you do not repair it. True end-to-end QAT (attn+MLP jointly trainable through the kernel) stays infra-blocked, not refuted.
...
## C9 MTP speculative decode on DeepSeek-V4-Flash-NVFP4 (2026-07-21): KILL
Branch `feat/mtp-spec-decode`, full write-up [c9/verdict.md](c9/verdict.md). Tested whether MTP spec-decode amortizes the captured-decode collective floor (90.8% of captured decode = per-layer TP all-reduce over PCIe, no NVLink) by verifying k+1 tokens in one target forward. Deployed config: `nvidia/DeepSeek-V4-Flash-NVFP4`, TP=4 EP, cap=128 max_seqs=2, dense-NVFP4 baseline, C4 custom one-shot all-reduce; two-run TTFT-subtracted `dtps=63/(wall64-wall1)`.
- **KILL.** Captured spec=2 = **47.792 tok/s vs no-spec 58.126 (C4 SOTA) = -17.8%**; captured spec=1 **deadlocks** (66 min at 0.00 tok/s in `generate->step->get_output` after clean graph capture — vLLM #40926-family MTP+DSA+MoE+MLA runtime hang). Eager confirms lossless integration (eager no-spec 7.165, spec=1 6.829, identical PPL 4.1222).
- **Why it loses:** DeepSeek's `mtp.0` draft head is NOT lightweight — it is a full decoder block (MLA + 256-expert MoE + DSA lightning indexer, kept unquantized). Each draft step costs ~a real layer's all-reduce + expert all-to-all, run k times/step, and acceptance is imperfect (vLLM warns k>1 lowers it on the recursive MTP layer). At batch 1-2 (spec-decode's best case) that outweighs the (k+1)x amortization of the target forward's 43 all-reduces. Integration is sound and lossless only in the eager spec=1 diagnostic (PPL 4.1222 = baseline); captured spec=2 measured PPL 4.2514 (a small regression) and captured spec=1 never completed. The loss is workload economics on this interconnect, not a bug.
- **Consequence:** deployed decode SOTA stays **C4 custom one-shot all-reduce 58.126 tok/s** — the collective floor is beaten by attacking it directly, and speculation is now proven not to beat that here. T4 (spec + sparse experts at M=B*k) not run: no captured win to amplify; the large-M sparse win lives in the crossover regime, not batch-1 decode.

## C10 sparse-FP4 MoE prefill serving throughput (2026-07-21): KILL — no crossover
Branch `c10-sparse-prefill`, full write-up [c10/verdict.md](c10/verdict.md). C2 settled dense fused NVFP4 MoE wins *decode* (8.1×) and flagged the prefill/large-M kernel Pareto as "not re-measured in serving." C10 measured it: `DeepSeek-V4-Flash-NVFP4`, 4-GPU EP, eager, end-to-end prefill tok/s (`P/TTFT`, new `--prefill-p` knob, prefix-cache-safe unique first token), all-sparse vs dense fused. Per-expert `M ~ P*6/256`, raised via single-chunk `max_len>=P`.
- **KILL, no crossover in the measured range.** Three paired points, sparse loses every one and the ratio *plateaus* not climbs: P2048 4735/7589 (0.62); P16384 6712/7904 (0.85); P32768 **6781**/7793 (0.87); then sparse turns down to P65536 6228 (−8%, dense OOM there = unmeasured). Sparse peak 6781 (M~768) is below dense floor 7589 (M~48) — curves never overlap over M~48-768. M>768 formally unmeasured (dense OOMs single-chunk) but the trend gives no path to a crossover (sparse already declining, dense flat-high 7589-7904).
- **Why:** dense fused `group_gemm_nvfp4` is flat (+2.7% over 16× M) = launch/bandwidth-saturated, not FLOP-bound, so 2:4 halving buys nothing to race. Sparse per-expert seg loop rises to M~768 then falls (routing overhead amortized then kernel-efficiency/activation-pressure rolloff). The kernel-microbench sparse-Pareto win (§5) does NOT survive vLLM integration: fused (not naive) dense baseline + prefill is not MoE-dominated (MLA/AR/norm in TTFT) + dual residency +26-27% wt.
- **Consequence:** sparse-FP4 MoE wins served throughput in **no** regime — decode (C2) and prefill (C10) both go to the dense fused kernel. The MoE sparse contribution is **quality/transfer only**, never a serving speed lever. Corrects the C9 note above: the large-M sparse win does not materialize in serving even in the compute-bound regime.

## B200 head-to-head: the SM120 decode numbers are platform-bound, not kernel-bound, 3.27x (runs 2026-07-23 UTC, written up 2026-07-24, branch `b200-glm-headtohead` PR #38, `docs/b200/verdict.md`)
Prompted by Aster (YC) advertising GLM-5.2 at **281 tok/s** against our frozen **33.810**. C4 had already attributed 90.8% of the SM120 decode step to one per-layer ring all-reduce over PCIe (~374 us for a ~14 KB payload = sync latency, not transfer) with GEMM+MoE at 2.2%, but that attribution had never been tested by changing the hardware. Pre-registered in `docs/b200/design.md` before results (bands: >=150 confirms, 60-150 partial, <60 refutes).
- **Same checkpoint, same harness, same passage (`ntok=96` both rows), same captured graph mode, same commit.** 8x RTX PRO 6000 (PCIe) **34.305 tok/s** (reproduces the frozen 33.810 within 1.5%, so not toolchain drift) vs **4x B200 (NVLink, vanilla vLLM, `QB_DENSE=off`) 112.079 tok/s = 3.27x**, both captured PASS, neither row collapsing on quality (PPL 3.9168 vs 3.7352, but 96 scored tokens cannot resolve a 0.18 delta so it is unattributed — NOT a quality claim in either direction).
- **This is a PLATFORM comparison, not a silicon-only A/B.** The B200 row also changes rank count (4 vs 8), plugin usage (`QB_DENSE=off` vs the SM120 patches), the DeepGEMM setting, and vLLM's NVFP4 MoE backend (`FLASHINFER_TRTLLM` vs `FLASHINFER_CUTLASS`). Each is the configuration that platform requires, but none is separated from the interconnect by this experiment. What IS held exactly fixed is quadbit: no kernel/quant/policy change, and on B200 quadbit is disabled outright.
- **Settles: the SM120 serving numbers were never a kernel deficit.** No kernel, quantization, or policy changed and the number tripled. Anyone reading quadbit's 33.810 as evidence about our kernels was reading the interconnect. B200 also fits the 433 GiB checkpoint on **4** GPUs (191.5 GB each) where SM120 needed 8, and 4xB200 is hourly-cost-matched to 8xRTX PRO 6000.
- **PARTIAL, not confirmed: the floor moved but did not vanish.** Predicted 170-230, got 112. Working backward from C4 (26.47 ms collective + 2.68 ms other on SM120) and holding the non-collective term constant, the B200 collective is **6.24 ms / 78 layers = ~80 us per all-reduce**, i.e. only **4.7x** cheaper than PCIe, leaving the collective **still ~70% of the B200 step**. Same pattern as C7 (remove one collective, another is promoted). The 80 us is **inferred, not measured** — a B200 `floor_profile` would settle it, but that harness is DeepSeek-hardcoded and needs a GLM+SM100 path. **So "buy NVLink" is not the whole answer either: decode stays collective-dominated, and the real lever is reducing collective count or overlapping it.**
- **The residual gap to 281 is published technique, not silicon or kernels.** Baseten (same 280+ claim, Artificial Analysis 2026-06-22) disclose the recipe: NVFP4 weights (we already use this, same format), **prefill/decode disaggregation = 2x on their own measurement** (112.079 x2 = 224), MTP speculative decoding on top, and KV-aware routing. We are an unoptimized research harness against a tuned production stack, and the distance is a list of implementable techniques with published multipliers.
- **C9 REVERSES on NVLink, at C9's own speculation count.** The MTP KILL (-17.8% on SM120) was driven by each draft step paying the per-layer 374 us all-reduce. At ~80 us it pays for itself twice over: **B200 spec=2 = 216.828 tok/s vs 112.079 no-spec = +93.5%** (SM120 spec=2 was **-17.8%**; only the machine differs), spec=1 = 181.278 (+61.7%), all **lossless** (PPL identical to 4dp, 3.7352 across every B200 row), all captured PASS. **Cumulative 34.305 -> 216.828 = 6.32x** from an interconnect change plus one config flag, no kernel/quant/policy change. Gap to Aster's 281 goes **8.19x -> 2.51x -> 1.30x**. Deeper speculation keeps paying on NVLink (spec2/spec1 = 1.196x) where SM120 warned k>1 lowers acceptance. **Caveat: this does NOT isolate the interconnect as the cause.** C9 was DeepSeek on SM120; these rows are GLM on B200, so model + hardware + platform config all differ, and DeepSeek's draft head (a full MoE+MLA+DSA block) is not equivalent to GLM's. The sign flip is *predicted by* and *consistent with* the interconnect explanation but does not prove it; the clean isolation test (**GLM-5.2 + MTP on SM120**, same model, same k, PCIe) is cheap and NOT yet run. Unconditional claim: on B200, MTP is a large lossless win for GLM-5.2 at batch 1-2. Also do NOT compose Baseten's 2x PD-disaggregation onto our number: it is measured on *their* concurrent-serving benchmark and works by keeping prefill from interrupting decode, so at single-stream batch 1-2 its expected contribution here is nearer nothing than 2x. And 281 itself is a **vendor marketing figure** whose measurement conditions we neither control nor reproduced — directional reference, not a calibrated baseline.
- **Campaign-wide consequence: the SM120 KILLs are conditional on the PCIe floor, not general.** C7 (DP-attention), C8 (pipeline staging) and C9 (MTP) all concluded "no software lever moves SM120 decode", and all were measured under a 374 us per-layer collective that was 90.8% of the step. C9 has now flipped on hardware alone. C7 and C8 are the obvious re-test candidates.

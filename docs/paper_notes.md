...
Settles the last open QAT question from WS-E ("only the gateup49 CONTROL stays PPL-only"; can QAT repair the gate_up tax?). Global-consistent gate_up-only STE-QAT layerwise repair (reio2 sparse-trajectory teacher, layers 22-42, 300 STE steps, lr=1e-4, down held dense-exact) on DeepSeek-V4-Flash-NVFP4, evaluated on the same WS-E 8-task limit=400 battery in-process.
- **8-task AVG (normalized primary):** one-shot gateup49 (no QAT, new control) **.7363**; **QAT gateup49 .7332 = -0.31pt** (within MC noise, SE ~0.8pt). Per-layer STE fit was tight (gate_up-vs-dense rel 0.02-0.07 all 21 layers) yet downstream did not move; only arc_c is outside noise and QAT is WORSE there.
- **Verdict: gate_up STE-QAT layerwise repair does NOT recover.** Same lesson as Gap C full-stack QAT (`docs/qat/design.md`) and A3 per-expert: local reconstruction rel does not predict downstream; the 2:4-FP4 gate_up capability floor is real, not a repair artifact. The result VALIDATES the deployed strategy - you AVOID the gate_up tax by anchoring gate_up dense (down49), you do not repair it. down49 stays the cleanest capability-preserving policy.
